...
async function meshes(file: string) {
  const bytes = fs.readFileSync(file), loader = new GLTFLoader().setMeshoptDecoder(MeshoptDecoder);
  ...
  const result = new Map<string, Vertex[]>(), position = new THREE.Vector3(), normal = new THREE.Vector3();
  scene.traverse(object => {
    if (!(object instanceof THREE.Mesh)) return;
    const { attributes } = object.geometry as THREE.BufferGeometry, normalMatrix = new THREE.Matrix3().getNormalMatrix(object.matrixWorld);
    ...
    for (let i = 0; i < attributes.position.count; i++) {
      ...
      if (attributes.normal) normal.fromBufferAttribute(attributes.normal, i).applyMatrix3(normalMatrix).normalize(); else normal.set(0, 0, 0);
      vertices.push([...position.toArray(), ...normal.toArray(), attributes.uv?.getX(i) ?? 0, attributes.uv?.getY(i) ?? 0] as Vertex);
    }
...
const original = await meshes(originalFile), encoded = await meshes(encodedFile);
const CELL = .002, cell = (value: number) => Math.round(value / CELL);
let positionMm = 0, normalDegrees = 0, uv = 0, unmatched = original.size !== encoded.size;
  ...
  for (const vertex of vertices) {
    ...
    for (let dx = -1; dx <= 1; dx++) for (let dy = -1; dy <= 1; dy++) for (let dz = -1; dz <= 1; dz++) {
      for (const candidate of grid.get(`${cell(vertex[0]) + dx},${cell(vertex[1]) + dy},${cell(vertex[2]) + dz}`) ?? []) {
        ...
        const degrees = THREE.MathUtils.radToDeg(Math.acos(THREE.MathUtils.clamp(vertex[3] * candidate[3] + vertex[4] * candidate[4] + vertex[5] * candidate[5], -1, 1)));
        ...
        const relative = mm / 1 + degrees / .25 + du / 5e-4;
        if (relative < score) { score = relative; best = [mm, degrees, du]; }
      }
    }
    if (!best) { unmatched = true; continue; }
    positionMm = Math.max(positionMm, best[0]); normalDegrees = Math.max(normalDegrees, best[1]); uv = Math.max(uv, best[2]);
  }
}
console.log(JSON.stringify(unmatched ? { positionMm: null, normalDegrees: null, uv: null } : { positionMm, normalDegrees, uv }));
