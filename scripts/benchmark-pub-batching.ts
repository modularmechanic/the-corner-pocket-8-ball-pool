...
scene.traverseVisible((object) => {
  ...
  const groups: { start: number; count: number; materialIndex?: number }[] = Array.isArray(object.material)
    ? object.geometry.groups
    : [
        {
          start: 0,
          count: object.geometry.index?.count ?? object.geometry.attributes.position.count,
          materialIndex: 0,
        },
      ];
  ...
  contributors.push({
    name: `${owner}/${object.name || list[0].name || object.geometry.type}`,
    triangles: Math.round(groups.reduce((sum, group) => sum + group.count / 3, 0) * instances),
    instances,
  });
  for (const group of groups) {
    ...
    drawCalls += passes;
    if (material.transparent) transparentDrawCalls += passes;
    triangles += (group.count / 3) * (object instanceof THREE.InstancedMesh ? object.count : 1);
  }
});
console.log(
  JSON.stringify(
    {
      fixture: 'current',
      loadedProps: settled.loaded.length,
      failedProps: settled.failed,
      meshes,
      drawCalls,
      ...
      constructionMs: Math.round(performance.now() - started),
      errors,
      topTriangleContributors: contributors.sort((a, b) => b.triangles - a.triangles).slice(0, 8),
