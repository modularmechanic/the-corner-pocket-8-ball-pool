...
export const DIFFICULTY_OPTIONS: (MenuOption<Difficulty> & { opponent: string })[] = entries(AI_NAMES).map(
  ([value, opponent]) => ({ value, label: value[0].toUpperCase() + value.slice(1), opponent }),
);
export const LAYOUT_OPTIONS: MenuOption<ArenaLayout>[] = entries(LAYOUTS).map(([value, layout]) => ({
  value,
  label: layout.name,
}));
const RULE_DETAILS = { old: 'Two shots after a foul', new: 'Ball in hand anywhere' } satisfies Record<RuleSet, string>;
export const RULE_OPTIONS: MenuOption<RuleSet>[] = entries(RULE_DETAILS).map(([value, detail]) => ({
  value,
  label: `${RULE_NAMES[value]} · ${detail}`,
}));
export const QUALITY_OPTIONS: MenuOption<RenderQuality>[] = entries(QUALITY_LABELS).map(([value, label]) => ({
  value,
  label,
}));
...
function validRecord(record: unknown): record is HouseRecord {
  ...
  return (
    ...
    Object.hasOwn(LAYOUTS, r.layout) &&
...
export class PlayerProfile {
  ...
  constructor(private readonly storage: ProfileStorage | null) {
    ...
    const volume = Number(this.read('volume') ?? 0.65);
    this.prefs = {
      sound: this.read('sound') !== 'false',
      volume: Number.isFinite(volume) ? Math.max(0, Math.min(1, volume)) : 0.65,
  ...
  levelOptions(): MenuOption[] {
    return Array.from({ length: MAX_LEVEL }, (_, index) => {
      const level = index + 1,
        locked = level > this.unlocked;
      return {
        value: String(level),
        label: `${level} · ${LEVEL_NAMES[index]}${locked ? ' · Locked' : ''}`,
        disabled: locked,
      };
    });
  }
  ...
  recordResult(
    ...
    if (
      ...
      return false;
    if (canAdvance(state, mode)) {
      this.unlocked = Math.max(this.unlocked, normalizeLevel(state.arcade.level + 1));
      this.write('unlocked-level', String(this.unlocked));
    }
    ...
    for (const team of resultTeams(mode, seat)) {
      ...
      records.push({
        id,
        name: teamName(team),
        score: state.arcade.scores[team],
        layout: state.arcade.layout,
        win: state.winner === team,
