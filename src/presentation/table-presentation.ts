...
export const AI_NAMES: Readonly<Record<Difficulty, string>> = {
  casual: 'The Newcomer',
  regular: 'The Regular',
  expert: 'The Hustler',
};
    ...
    return seat === 0
      ...
      : seat === 2
        ...
        : state.format === 'doubles'
          ? `${AI_NAMES[viewer.difficulty]} ${seat === 1 ? 'A' : 'B'}`
          : AI_NAMES[viewer.difficulty];
...
export function teamLabel(
  ...
  return state.format === 'doubles'
    ? viewer.mode === 'ai'
      ? team === 0
        ? 'Your team'
        : 'House team'
      : `Team ${team + 1}`
    : seatLabel(state, viewer, team);
}
...
export function deriveTableEffects(state: GameState) {
  const teams = ([0, 1] as const).map((team) => {
    ...
    const queued = Object.fromEntries(STATUS_IDS.map((id) => [id, !!state.arcade?.buffs[team][id]])) as Record<
      StatusEffect,
      boolean
    >;
    const active = Object.fromEntries(
      STATUS_IDS.map((id) => [id, shooting ? id !== 'jammed' && !!shot?.[id] : preparing && queued[id]]),
    ) as Record<StatusEffect, boolean>;
    const pills: EffectPill[] = [];
    const append = (id: EffectId, on: boolean, next: boolean) => {
      const definition = EFFECTS[id];
      const suffix =
        shooting && next ? (on ? ' · Active; next shot queued' : ' · Next shot') : !on && next ? ' · Next shot' : '';
      pills.push({
        id,
        title: definition.name + suffix,
    ...
    if (state.chalked[team] || shot?.chalked)
      append('chalked', !!shot?.chalked || (preparing && state.chalked[team]), state.chalked[team]);
...
export function deriveTablePresentation(state: GameState, viewer: TableViewer) {
  const effects = deriveTableEffects(state),
    actor = seatLabel(state, viewer, activeSeat(state));
  ...
  if (viewer.resetting) text = 'Racking up';
    ...
    text = !viewer.connected
      ? 'Reconnecting…'
      : (viewer.room?.players.length || 0) < seatCount(state.format)
  ...
  return {
    status: {
      text,
      waiting: waiting || state.phase === 'rolling' || !!viewer.aiThinking,
      foul: state.phase === 'ball-in-hand' || allowance,
    },
    rules: RULE_NAMES[state.rules],
    effects,
    teams: ([0, 1] as const).map((team) => ({
      name: teamLabel(state, viewer, team),
      pills: effects.teams[team].pills,
    })),
