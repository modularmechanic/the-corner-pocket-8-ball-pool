...
export interface ShotPreview {
  ...
  corridor: (PreviewSegment | null)[];
  fan: PreviewFan | null;
}
const offset = (point: Point, dx: number, dz: number, distance: number): Point => ({
...
export function shotGuideRisk(power: number, cutAngle: number): number {
  const stroke = clamp(Number.isFinite(power) ? power : 0.65, 0, 1);
  const cut = Math.sin(clamp(Number.isFinite(cutAngle) ? Math.abs(cutAngle) : 0, 0, Math.PI / 2));
  return 1 - (1 - 0.62 * stroke * stroke) * (1 - 0.82 * Math.pow(cut, 1.3));
}
...
export function shotPreview(state: GameState, shot: PreviewShot, focus = false): ShotPreview {
  const preview: ShotPreview = { contact: null, ghost: null, segments: [], corridor: [null, null], fan: null },
    cue = state.balls.find((ball) => ball.id === 0);
  ...
  const contact = firstTableContact(state, cue, shot.angle, 0);
  preview.contact = contact;
  const approachEnd = Math.max(0, contact.distance - (contact.kind === 'ball' ? TABLE.radius : 0));
  if (approachEnd > TABLE.radius + 0.025)
    preview.segments.push({
      kind: 'approach',
      from: offset(cue, dx, dz, TABLE.radius + 0.018),
      to: offset(cue, dx, dz, approachEnd),
      strength: 1,
    });
  ...
  for (let i = 0; i < 2; i++) {
    ...
    const next = firstTableContact(state, edge, shot.angle, 0),
      end = Math.min(contact.distance, next.distance),
      start = TABLE.radius + 0.018;
    if (end > start + 0.025)
      preview.corridor[i] = {
        kind: 'approach',
        from: offset(edge, dx, dz, start),
        to: offset(edge, dx, dz, end),
        strength: 1,
      };
  }
  ...
  if ((struck.elevation ?? 0) > 0.03 || (cue.elevation ?? 0) > 0.03) return preview;
  const gap = Math.hypot(struck.x - contact.x, struck.z - contact.z);
  if (gap < 1e-8) return preview;
  const nx = (struck.x - contact.x) / gap,
    nz = (struck.z - contact.z) / gap;
  const normal = Math.max(0, dx * nx + dz * nz);
  const tangentX = dx - nx * normal,
    tangentZ = dz - nz * normal;
  ...
  const append = (kind: 'object' | 'cue', origin: Point, vx: number, vz: number, cap: number) => {
    ...
    const ux = vx / strength,
      uz = vz / strength;
    const next = firstTableContact(continuing, origin, Math.atan2(uz, ux), kind === 'object' ? struck.id : 0);
    const length = Math.min(next.distance, cap * strength),
      start = TABLE.radius + 0.025;
    if (length <= start + 0.025) return;
  ...
  if (object) {
    const cutAngle = Math.acos(clamp(normal, 0, 1)),
      spread = (focus ? 0.9 : 0.66) * Math.sin(cutAngle);
    const length = Math.hypot(object.to.x - object.from.x, object.to.z - object.from.z);
    const fan: PreviewFan = { cutAngle, spread, risk: shotGuideRisk(shot.power ?? 0.65, cutAngle), rays: [] };
    for (let i = 0; i < FAN_RAY_COUNT; i++) {
      ...
      const cap = Math.max(0, Math.min(desired, next.distance - 0.02));
    ...
    if (length > 0.025) preview.fan = fan;
  }
  return preview;
}
