...
import { groupOf, other, seededRandom, TABLE, type GameState, type ShotResult, type StatusEffect, type TableEvent } from './types';

export interface SettlementContext { legalBefore:readonly number[]; shooter?:0|1; pendingPortal?:boolean }
export interface RespotTarget { id:number;x:number;z:number }
export interface SettlementOutcome {
  shooter:0|1; foul:boolean; scratched:boolean; wardRescued:boolean;
  ownPotted:number; opponentPotted:number; respotEight:boolean; winner:0|1|null;
}
export interface ShotSettlement { state:GameState; outcome:SettlementOutcome; respots:RespotTarget[]; events:Omit<TableEvent,'time'>[] }
...
export function settleShot(input:GameState,result:ShotResult,context:SettlementContext):ShotSettlement {
  const state=structuredClone(input),shot=structuredClone(result);
  delete state.simulation;
  const shooter=context.shooter??state.turn,arcade=state.arcade;
  const respots:RespotTarget[]=[],events:Omit<TableEvent,'time'>[]=[];
  const offTable=shot.offTable??[],isBreak=state.shotCount===0;
  // The physical result is authoritative even for a rule-only caller.
  for(const id of [...shot.potted,...offTable])if(state.balls[id]) {
  ...
  const respot=(id:number,points:Iterable<{x:number;z:number}>,policy:BallClearancePolicy):boolean=>{
    for(const point of points)if(isClearBallSpot(state,point,id,policy)) {
      Object.assign(state.balls[id],point,{pocketed:false,vx:0,vz:0,vy:0,elevation:0,airborne:false});
      respots.push({id,...point});return true;
    }
    return false;
  };
  ...
  const eightDown=shot.potted.includes(8)||offTable.includes(8),respotEight=isBreak&&eightDown;
  state.shotCount++;state.lastPotted=shot.potted.filter(id=>id!==0);state.foul=foul;state.turn=shooter;
  if(eightDown&&!isBreak) {
    const legalEight=context.legalBefore.length===1&&context.legalBefore[0]===8&&!foul;
    state.winner=legalEight?shooter:other(shooter);state.phase='over';
    state.message=offTable.includes(8)?'The eight left the table. Rack lost.':legalEight?'Eight ball down. Beautifully played.':scratched?'The eight and the cue ball went down. Rack lost.':'The eight went down too early. Rack lost.';
  } else {
    if(!isBreak&&!state.groups[shooter]&&!foul) {
      const first=shot.potted.find(id=>groupOf(id));
      if(first!==undefined){state.groups[shooter]=groupOf(first);state.groups[other(shooter)]=groupOf(first)==='solids'?'stripes':'solids';}
    }
  }
  // One count feeds turn retention, score, combo and streak outcomes.
  const group=state.groups[shooter];
  let ownPotted=0,opponentPotted=0;
  ...
  if(arcade) {
    ...
    if(!foul)arcade.scores[shooter]+=ownPotted*100+25*ownPotted*(ownPotted-1)/2+(state.winner===shooter?500:0);
    ...
    const award=(choices:StatusEffect[],reason:NonNullable<TableEvent['reason']>)=>{
      ...
      const status=choices[Math.floor(random()*choices.length)];arcade.buffs[shooter][status]=1;
      const cue=state.balls[0];events.push({kind:'status',status,reason,x:cue.x,z:cue.z,strength:.8});
    };
  ...
  if(arcade) {
    ...
    if(arcade.portalTurns===0||state.winner!==null){arcade.hazards=arcade.hazards.filter(h=>h.kind!=='portal');arcade.portalTurns=0;}
  }
  return {state,respots,events,outcome:{shooter,foul,scratched,wardRescued,ownPotted,opponentPotted,respotEight,winner:state.winner}};
}

function* wardSpots(){for(let x=-2.85;x<5.2;x+=.4)for(let z=-2.3;z<2.4;z+=.4)yield {x,z};}
function* eightSpots(){for(let x=2.55;x>-5.3;x-=TABLE.radius*2.1)yield {x,z:0};}
function* objectSpots(){for(let ring=0;ring<28;ring++)for(let i=0;i<Math.max(1,ring*8);i++){const angle=i/Math.max(1,ring*8)*Math.PI*2;yield {x:2.55+Math.cos(angle)*ring*.38,z:Math.sin(angle)*ring*.38};}}
