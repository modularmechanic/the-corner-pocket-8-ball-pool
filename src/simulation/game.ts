...
function readOnlyView<T extends object>(source:T):T {
  ...
  const view=(target:object):object=>{
    const existing=views.get(target);if(existing)return existing;
    const proxy=new Proxy(target,{
      get(object,key){const value=Reflect.get(object,key);return value&&typeof value==='object'?view(value):value;},
      set(){throw new TypeError('PoolGame.state is read-only; edit snapshot() and call arrange().');},
      deleteProperty(){throw new TypeError('PoolGame.state is read-only; edit snapshot() and call arrange().');},
      defineProperty(){throw new TypeError('PoolGame.state is read-only; edit snapshot() and call arrange().');},
      setPrototypeOf(){throw new TypeError('PoolGame.state is read-only; edit snapshot() and call arrange().');},
      preventExtensions(){throw new TypeError('PoolGame.state is read-only; edit snapshot() and call arrange().');},
    });views.set(target,proxy);return proxy;
...
export class PoolGame {
  ...
  arrange(input: GameState): void {
    ...
    for(const ball of state.balls) {
      ball.elevation=ball.elevation??0;ball.vy=ball.vy??0;
      ball.airborne=!ball.pocketed&&(ball.elevation>FLIGHT_EPSILON||Math.abs(ball.vy)>.18);
      if(ball.pocketed){ball.vx=0;ball.vz=0;ball.vy=0;}
    }
    this.world?.free();this.events.free();this.events=new RAPIER.EventQueue(true);
    this.bodies.clear();this.colliderIds.clear();this.colliderKeys.clear();this.obstacleColliders.clear();
    this.activeContacts=new Set(continuation?.activeContacts??[]);
    this.current=state;this.stateView=readOnlyView(state);
    this.shotResult=continuation?.shotResult??{firstContact:null,potted:[],railAfterContact:false,breakRails:[],offTable:[]};
    this.legalBefore=continuation?.legalBefore??legalTargets(state).map(b=>b.id);
    this.stillTime=continuation?.stillTime??0;this.rollTime=continuation?.rollTime??0;
    ...
    this.cueSpin=continuation?.cueSpin??{side:0,follow:0,energy:0,elevation:0,tipY:0,dx:1,dz:0,contact:false};
    ...
    this.pickupRandom=()=>{this.pickupDraws++;return random();};
    this.nextPickupAt=continuation?.nextPickupAt??(state.arcade?.clock??0)+5+this.pickupRandom()*4;
    this.nextPickupId=continuation?.nextPickupId??Math.max(2,...(state.arcade?.pickups??[]).map(p=>p.id+1));
    this.world = new RAPIER.World({ x: 0, y: 0, z: 0 });
    this.world.timestep = 1 / 120;
    this.world.numSolverIterations = 12;
    for(const [id,rail] of TABLE_RAILS.entries()) {
      const collider=this.world.createCollider(RAPIER.ColliderDesc.cuboid(rail.halfWidth,rail.halfHeight,rail.halfDepth).setTranslation(rail.x,rail.y,rail.z).setRestitution(rail.restitution).setFriction(0).setActiveEvents(RAPIER.ActiveEvents.COLLISION_EVENTS));
    ...
    for(const [id,nose] of TABLE_NOSES.entries()) {
      const collider=this.world.createCollider(RAPIER.ColliderDesc.ball(nose.radius).setTranslation(nose.x,nose.y,nose.z).setRestitution(nose.restitution).setFriction(0).setActiveEvents(RAPIER.ActiveEvents.COLLISION_EVENTS));
      this.colliderIds.set(collider.handle,-1);
    ...
    for (const ball of this.current.balls) {
      ...
      const body = this.world.createRigidBody(RAPIER.RigidBodyDesc.dynamic().setTranslation(ball.x, TABLE.radius+(ball.elevation||0), ball.z).lockRotations().setCcdEnabled(true).setCanSleep(true));
      const collider = this.world.createCollider(RAPIER.ColliderDesc.ball(TABLE.radius).setRestitution(.96).setRestitutionCombineRule(RAPIER.CoefficientCombineRule.Min).setFriction(0).setDensity(1).setActiveEvents(RAPIER.ActiveEvents.COLLISION_EVENTS), body);
    ...
    for (const obstacle of this.current.arcade?.obstacles || []) {
      ...
      const collider = this.world.createCollider(RAPIER.ColliderDesc.cuboid(obstacle.width/2,.26,obstacle.depth/2).setTranslation(obstacle.x,.2,obstacle.z).setRestitution(.74).setFriction(0).setActiveEvents(RAPIER.ActiveEvents.COLLISION_EVENTS));
      this.colliderIds.set(collider.handle,-2-obstacle.id); this.obstacleColliders.set(obstacle.id,collider.handle);
      this.colliderKeys.set(collider.handle,`obstacle:${obstacle.id}`);
    }
  }
  private emit(event: Omit<TableEvent, 'time'>) { this.onEvent?.({ ...event, time: this.rollTime }); }
  chalkCue():boolean {
    ...
    const cue=this.current.balls[0];this.emit({kind:'chalk',x:cue.x,z:cue.z,strength:.65,ball:0});
    return true;
  }
  equipCue(seat: number, cue: unknown): boolean {
    if (!['ready', 'ball-in-hand'].includes(this.current.phase) || !Number.isInteger(seat) || seat < 0 || seat >= this.current.cues.length || !canEquipCue(cue, this.current.arcade?.level)) return false;
    this.current.cues[seat] = cue;
    return true;
  }
  shoot(shot: Shot): boolean {
    if (this.current.phase !== 'ready' || !Number.isFinite(shot?.angle) || !Number.isFinite(shot?.power) || shot.power < .03 || shot.power > 1) return false;
    const elevation=shot.elevation===undefined?0:shot.elevation,tipX=shot.tipX===undefined?0:shot.tipX,tipY=shot.tipY===undefined?0:shot.tipY;
    if(![elevation,tipX,tipY].every(Number.isFinite)||elevation<0||elevation>Math.PI/3||Math.hypot(tipX,tipY)>.80000001)return false;
    ...
    this.current.lastPotted = []; this.current.foul = false;
    this.current.phase = 'rolling'; this.current.message = 'A little patience. Let the table do its thing.';
    this.stillTime = 0; this.rollTime = 0; this.obstacleCooldown.clear(); this.zoneOccupants.clear(); this.portalCooldown.clear(); this.hazardRewards.clear(); if(this.current.arcade)this.current.arcade.combo=0;
    const arcade=this.current.arcade, buffs=arcade?.buffs[this.current.turn];
    if(arcade && buffs) { arcade.activeShot={overdrive:buffs.overdrive>0,frozen:buffs.frozen>0,ward:buffs.ward>0,focus:buffs.focus>0,sticky:buffs.sticky>0,chalked,elevation,tipX,tipY}; buffs.overdrive=0;buffs.frozen=0;buffs.ward=0;buffs.focus=0;buffs.jammed=0;buffs.sticky=0; }
    const equipment = equippedCue(this.current);
    const efficiency=1-(chalked ? .04 : .22)*(tipX*tipX+tipY*tipY)/.64;
    const strikeSpeed = Math.min(23, (1.4 + shot.power * 15) * equipment.power * (arcade?.activeShot.overdrive ? 1.3 : 1) * (arcade?.activeShot.frozen ? .65 : 1)*efficiency);
    ...
    const hard = Math.max(0,(shot.power-.55)/.45), low = Math.max(0,(-tipY-.3)/.5);
    const rawLift = Math.min(5.4,(4.2*low*Math.pow(hard,.85)+3.1*Math.sin(elevation)**2*Math.max(0,(shot.power-.4)/.6))*(chalked?1.05:1));
    const lift=rawLift>.45?rawLift:0;
    const flatSpeed = strikeSpeed*Math.cos(elevation);
    const speed = Math.sqrt(Math.max(flatSpeed*flatSpeed*.45,flatSpeed*flatSpeed-lift*lift*.75));
    cue.vx = Math.cos(shot.angle) * speed; cue.vz = Math.sin(shot.angle) * speed;
    cue.vy=lift;cue.elevation=0;cue.airborne=lift>.1;
    const grip=(chalked?1.12:.8)*equipment.spin,side=tipX*speed*.13*grip,follow=tipY*speed*.26*grip*(1+.55*Math.sin(elevation));
    this.cueSpin={side,follow,energy:side*side+follow*follow,elevation,tipY,dx:Math.cos(shot.angle),dz:Math.sin(shot.angle),contact:false};
    ...
    if(cue.airborne)this.emit({kind:'jump',strength:Math.min(1,lift/5.4),x:cue.x,z:cue.z,ball:0,elevation:0});
    return true;
  }
  ...
  private respot(id: number, x: number, z: number) {
    ...
    ball.x = x; ball.z = z; ball.vx = 0; ball.vz = 0; ball.vy=0;ball.airborne=false;ball.elevation=0; ball.pocketed = false;
    const body = this.bodies.get(id)!;
    body.setEnabled(true); body.setTranslation({ x, y: TABLE.radius, z }, true); body.setLinvel({ x: 0, y: 0, z: 0 }, true);
  }
  ...
  advanceIdle(seconds:number):void {
    if(!Number.isFinite(seconds)||seconds<=0||this.current.phase==='rolling'||this.current.phase==='over'||!this.current.arcade)return;
    let remaining=seconds;
    while(remaining>1e-9) {
      const arcade=this.current.arcade;
      const deadline=Math.min(this.nextPickupAt,...arcade.pickups.filter(p=>p.available&&p.expiresAt!==undefined).map(p=>p.expiresAt!));
      const untilEvent=Math.max(1/120,Math.ceil((deadline-(arcade.clock??0))*120-1e-8)/120);
      const elapsed=Math.min(remaining,untilEvent);this.advancePickups(elapsed);remaining-=elapsed;
    }
  }
  private integrateStep(dt: number): boolean {
    if (this.current.phase !== 'rolling') return false;
    const energyBefore=this.current.balls.reduce((sum,b)=>sum+(b.pocketed?0:b.vx*b.vx+b.vz*b.vz+(b.vy||0)**2+2*GRAVITY*(b.elevation||0)),0);
    this.rollTime += dt;
    this.world.timestep = dt;
    // Symmetric gravity integration conserves flight energy between collisions.
    for(const ball of this.current.balls)if(!ball.pocketed&&ball.airborne) {
      const body=this.bodies.get(ball.id)!;const velocity=body.linvel();
    ...
    this.events.drainCollisionEvents((a, b, started) => {
      ...
      if(!started){this.activeContacts.delete(key);return;}
      ...
      const ia = this.colliderIds.get(a), ib = this.colliderIds.get(b);
      if (ia === undefined || ib === undefined) return;
      if (ia < -1 || ib < -1) {
        const obstacleId = -(ia < -1 ? ia : ib)-2, ballId=ia>=0?ia:ib;
        if(ballId<0) return;
        const ball=this.current.balls[ballId], obstacle=this.current.arcade!.obstacles.find(o=>o.id===obstacleId)!;
        const speed=Math.hypot(ball.vx,ball.vz);
        this.shotResult.obstacleContact=true;
        if(speed>.55 && this.rollTime-(this.obstacleCooldown.get(obstacleId)??-1)>.12 && obstacle.hp>0) {
          ...
          this.emit({kind:'obstacle',strength:Math.min(1,speed/10),x:ball.x,z:ball.z,obstacle:obstacleId,destroyed:obstacle.hp===0});
          if(obstacle.hp===0) {
            const handle=this.obstacleColliders.get(obstacleId)!; const collider=this.world.getCollider(handle); if(collider)this.world.removeCollider(collider,true);
            this.colliderIds.delete(handle);this.obstacleColliders.delete(obstacleId);
            if(this.current.arcade!.activeShot.overdrive) this.bodies.get(ballId)!.setLinvel({x:ball.vx*.72,y:this.bodies.get(ballId)!.linvel().y,z:ball.vz*.72},true);
            const arcade=this.current.arcade!;arcade.destroyed[this.current.turn]++;arcade.scores[this.current.turn]+=obstacle.maxHp*50;
      ...
      if((ia===0&&ib===-1)||(ib===0&&ia===-1)) {
        const cue=this.current.balls[0],nx=Math.abs(cue.x)>5.3?Math.sign(cue.x):0,nz=nx?0:Math.sign(cue.z);
        this.pendingSpin.x-=nz*this.cueSpin.side*.8;
        this.pendingSpin.z+=nx*this.cueSpin.side*.8;
        this.cueSpin.side*=.6;
      }
      ...
      const ba=this.current.balls[ia>=0?ia:ib], bb=ia>=0&&ib>=0?this.current.balls[ib]:null;
      let speed=0;
      if(bb) {const dx=bb.x-ba.x,dy=(bb.elevation||0)-(ba.elevation||0),dz=bb.z-ba.z,len=Math.hypot(dx,dy,dz)||1;speed=Math.abs(((ba.vx-bb.vx)*dx+((ba.vy||0)-(bb.vy||0))*dy+(ba.vz-bb.vz)*dz)/len);}
      else speed=Math.abs(ba.x)>5.3?Math.abs(ba.vx):Math.abs(ba.vz);
      if(speed>.12) this.emit({kind:bb?'ball':'cushion',strength:Math.min(1,speed/11),x:bb?(ba.x+bb.x)/2:ba.x,z:bb?(ba.z+bb.z)/2:ba.z,ball:ba.id,elevation:ba.elevation||0});
    });
    let moving = false;
    for (const ball of this.current.balls) {
      ...
      let landed=false;
      if(p.y<=TABLE.radius+FLIGHT_EPSILON&&vy<=.18) {
        landed=wasAirborne;
        ...
        const rebound=vy < -2.4 ? -vy*.13 : 0;
        height=0;vy=rebound>.45?rebound:0;
        body.setTranslation({x:p.x,y:TABLE.radius,z:p.z},false);
        if(landed)this.emit({kind:'land',strength:Math.min(1,Math.abs(v.y)/5),x:p.x,z:p.z,ball:ball.id,elevation:0});
      }
      ball.elevation=height;ball.vy=vy;ball.airborne=height>FLIGHT_EPSILON||vy>.18;
      if(ball.airborne&&!wasAirborne)this.emit({kind:'jump',strength:Math.min(1,Math.max(vy,Math.sqrt(2*GRAVITY*height))/5.4),x:p.x,z:p.z,ball:ball.id,elevation:height});
      ...
      if (pocket || out) {
        ball.pocketed = true; ball.vx = 0; ball.vz = 0;ball.vy=0;ball.airborne=false;body.setEnabled(false);
        if(pocket)this.shotResult.potted.push(ball.id);
        else (this.shotResult.offTable??=[]).push(ball.id);
        this.emit({kind:pocket?'pocket':'out',strength:Math.min(1,.45+Math.hypot(v.x,v.y,v.z)/15),x:p.x,z:p.z,ball:ball.id,elevation:height}); continue;
      }
      const speed = Math.hypot(v.x, v.z);
      const zoneDrag=ball.airborne?1:surfaceDragAt(this.current.arcade,{x:p.x,z:p.z});
      const drag=ball.airborne ? speed*AIR_DRAG : rollingDeceleration(speed,zoneDrag*(ball.id===0&&this.current.arcade?.activeShot.sticky?STICKY_DRAG:1));
      const next = Math.max(0, speed - dt * drag)*(landed?.985:1);
      const ratio = speed > 0 ? next / speed : 0;
      ball.vx = v.x * ratio; ball.vz = v.z * ratio;
      if (next < .035 && !ball.airborne) { ball.vx = 0; ball.vz = 0; } else moving = true;
    ...
    const kineticAfter=this.current.balls.reduce((sum,b)=>sum+(b.pocketed?0:b.vx*b.vx+b.vz*b.vz+(b.vy||0)**2),0);
    const potentialAfter=this.current.balls.reduce((sum,b)=>sum+(b.pocketed?0:2*GRAVITY*(b.elevation||0)),0);
    if(kineticAfter+potentialAfter > energyBefore && kineticAfter > 0) {
      ...
      for(const ball of this.current.balls) if(!ball.pocketed) {
        ball.vx*=scale;ball.vz*=scale;ball.vy=(ball.vy||0)*scale;
        const body=this.bodies.get(ball.id)!;
        if(!body.isSleeping())body.setLinvel({x:ball.vx,y:ball.vy,z:ball.vz},false);
      }
    }
    this.applyCueSpin(dt);
    for(const ball of this.current.balls)if(!ball.pocketed){this.applyHazards(ball);this.collectPickups(ball);}
    this.stillTime = moving ? 0 : this.stillTime + dt;
    if (this.stillTime > .18 || this.rollTime > 28) {
      for (const b of this.current.balls) { b.vx = 0; b.vz = 0;b.vy=0;b.airborne=false; if(!b.pocketed)b.elevation=0;const body=this.bodies.get(b.id)!;body.setTranslation({x:b.x,y:TABLE.radius+(b.elevation||0),z:b.z},false);body.setLinvel({ x: 0, y: 0, z: 0 }, false); body.sleep(); }
      const settled=settleShot(this.current,this.shotResult,{legalBefore:this.legalBefore,shooter:this.current.turn,pendingPortal:this.portalRewardPending});
      this.current=settled.state;this.stateView=readOnlyView(this.current);
  ...
  private applyCueSpin(dt:number) {
    const cue=this.current.balls[0],spin=this.cueSpin;
    if(cue.pocketed){this.pendingSpin={x:0,z:0};return;}
    ...
    const curve=cue.airborne?0:spin.side*Math.sin(spin.elevation)*.14*(1+Math.max(0,-spin.tipY)*.7)*equippedCue(this.current).curve*dt;
    if(Math.abs(curve)>1e-9) {
      const x=cue.vx,z=cue.vz;
      cue.vx=x*Math.cos(curve)-z*Math.sin(curve);cue.vz=x*Math.sin(curve)+z*Math.cos(curve);
    }
    if(!cue.airborne&&(this.pendingSpin.x||this.pendingSpin.z)) {
      const before=cue.vx*cue.vx+cue.vz*cue.vz;
      let x=cue.vx+this.pendingSpin.x,z=cue.vz+this.pendingSpin.z;
      const requested=x*x+z*z,allowed=Math.min(23*23,before+spin.energy);
      if(requested>allowed&&requested>0){const scale=Math.sqrt(allowed/requested);x*=scale;z*=scale;}
      spin.energy=Math.max(0,spin.energy-Math.max(0,x*x+z*z-before));
      cue.vx=x;cue.vz=z;this.pendingSpin={x:0,z:0};
    }
    const body=this.bodies.get(0)!;
    if(Math.hypot(cue.vx,cue.vz)>.001)body.setLinvel({x:cue.vx,y:cue.vy||0,z:cue.vz},true);
    spin.follow*=Math.exp(-(cue.airborne?.12:.9)*dt);spin.side*=Math.exp(-(cue.airborne?.08:.3)*dt);spin.energy*=Math.exp(-(cue.airborne?.12:.8)*dt);
  }
  private addPickup(x:number,z:number) {
    ...
    if(active.length>=3) {
      ...
      oldest.available=false;this.emit({kind:'expire',x:oldest.x,z:oldest.z,pickup:oldest.id,power:oldest.power,strength:.35});
    }
  ...
  private advancePickups(dt:number) {
    ...
    for(const pickup of arcade.pickups)if(pickup.available&&pickup.expiresAt!==undefined&&arcade.clock>=pickup.expiresAt) {
      pickup.available=false;this.emit({kind:'expire',x:pickup.x,z:pickup.z,pickup:pickup.id,power:pickup.power,strength:.35});
    }
  ...
  private collectPickups(ball:Ball) {
    const arcade=this.current.arcade;if(!arcade||ball.id!==0||ball.airborne||Math.hypot(ball.vx,ball.vz)<.25)return;
    for(const pickup of arcade.pickups) {
      if(!pickup.available||Math.hypot(ball.x-pickup.x,ball.z-pickup.z)>pickup.radius+TABLE.radius)continue;
      pickup.available=false;
      const power=pickup.power??makePickup(this.current.seed,pickup.id,pickup.x,pickup.z,arcade.clock||0).power!;
      ...
      this.emit({kind:'pickup',power,reason:'pickup',pickup:pickup.id,x:pickup.x,z:pickup.z,ball:ball.id,strength:1});
    }
  }
  private applyHazards(ball: Ball) {
    ...
    for(const hazard of arcade.hazards) {
      ...
      const speed=Math.hypot(ball.vx,ball.vz);
      if(previous.has(hazard.id)||speed<.25)continue;
      if(hazard.kind==='portal') {
        ...
        const heading=Math.atan2(ball.vz,ball.vx),distance=destination.radius+TABLE.radius+.10;
        for(const offset of [0,Math.PI/4,-Math.PI/4,Math.PI/2,-Math.PI/2,Math.PI]) {
          const angle=heading+offset,x=destination.x+Math.cos(angle)*distance,z=destination.z+Math.sin(angle)*distance;
          if(!isClearBallSpot(this.current,{x,z},ball.id,'portal-exit'))continue;
          const fromX=ball.x,fromZ=ball.z;ball.x=x;ball.z=z;ball.vx=Math.cos(angle)*speed;ball.vz=Math.sin(angle)*speed;ball.vy=0;ball.elevation=0;ball.airborne=false;ball.teleport=(ball.teleport||0)+1;
          const body=this.bodies.get(ball.id)!;body.setTranslation({x,y:TABLE.radius,z},true);body.setLinvel({x:ball.vx,y:0,z:ball.vz},true);
          this.portalCooldown.set(ball.id,this.rollTime+.8);occupied.add(destination.id);
          this.emit({kind:'hazard',hazard:'portal',x,z,fromX,fromZ,strength:.8,ball:ball.id});break;
        }
        break;
      }
      if(hazard.kind==='ramp'||hazard.kind==='electric') {
        const along=ball.vx*Math.cos(hazard.angle||0)+ball.vz*Math.sin(hazard.angle||0);
        if(hazard.kind==='ramp'&&along<speed*.25)continue;
        const nextSpeed=Math.min(23,speed*(hazard.kind==='ramp'?1.14:1.18));
        ball.vx*=nextSpeed/speed;ball.vz*=nextSpeed/speed;
        if(hazard.kind==='ramp') {
          ball.vy=Math.min(2.45,2.05+speed*.02);ball.airborne=true;
          this.emit({kind:'jump',x:ball.x,z:ball.z,ball:ball.id,elevation:ball.elevation||0,strength:ball.vy/5.4});
        }
      ...
      if((hazard.kind==='electric'||hazard.kind==='ramp')&&!this.hazardRewards.has(rewardKey)) {arcade.scores[this.current.turn]+=15;this.hazardRewards.add(rewardKey);}
      this.emit({kind:'hazard',hazard:hazard.kind,x:ball.x,z:ball.z,strength:Math.min(1,.3+speed/15),ball:ball.id});
    }
