...
export class ShotCameraAim {
  ...
  angleAt(clientX: number, width: number, currentAngle: number): number {
    if (!Number.isFinite(clientX) || !Number.isFinite(width) || width <= 0 || !Number.isFinite(currentAngle))
      return currentAngle;
...
export function shotCameraPose(
  state: CameraState,
  shot: Pick<Shot, 'angle' | 'elevation'>,
  aspect: number,
): ShotCameraPose {
  ...
  const pitch = downToCue - Math.atan(Math.tan(THREE.MathUtils.degToRad(fov / 2)) * 0.47);
  const lookDistance = distance + 2.35;
  const target = new THREE.Vector3(
    position.x + dx * lookDistance,
    position.y - Math.tan(pitch) * lookDistance,
    position.z + dz * lookDistance,
  );
  return { position, target, fov, near: 0.035, far: 120, mode: 'shoot' };
}
...
export function watchCameraPose(
  heading: number,
  aspect: number,
...
export class ShotCameraRig {
  ...
  get isSettled(): boolean {
    return this.initialized && this.settled;
  }
  reset(preserveWatch = false): void {
    this.initialized = false;
    this.transition = null;
    this.settled = false;
    if (!preserveWatch) this.watchAnchored = false;
  }
  update(
    ...
    if (watch && (!this.initialized || this.mode !== 'watch')) {
      if (this.initialized) {
        ...
        this.watchTarget.copy(this.target);
        this.watchTarget.y = -0.12;
      } else if (!this.watchAnchored) {
        this.watchHeading = state.lastShot?.angle ?? shot.angle;
        this.watchTarget.set(0, -0.12, 0);
      }
    ...
    const relocation =
      pose.mode === 'shoot' &&
      ((cue.teleport ?? 0) !== this.teleport || Math.hypot(cue.x - this.cuePosition.x, cue.z - this.cuePosition.z) > 2);
    const snap = !this.initialized;
    if (!snap && (this.mode !== pose.mode || state.seed !== this.seed || relocation))
      this.transition = {
        position: camera.position.clone(),
        quaternion: camera.quaternion.clone(),
        lookDistance: camera.position.distanceTo(this.target),
        fov: camera.fov,
        age: 0,
        duration: watch ? SHOT_WATCH_TRANSITION : SHOT_RETURN_TRANSITION,
      };
    if (snap) this.transition = null;
    this.initialized = true;
    this.seed = state.seed;
    this.mode = pose.mode;
    this.teleport = cue.teleport ?? 0;
    this.cuePosition.set(cue.x, 0, cue.z);
    camera.aspect = aspect;
    camera.near = pose.near;
    camera.far = pose.far;
    camera.up.set(0, 1, 0);
    const rotation = new THREE.Quaternion().setFromRotationMatrix(
      new THREE.Matrix4().lookAt(pose.position, pose.target, camera.up),
    );
    let lookDistance = pose.position.distanceTo(pose.target);
    if (this.transition) {
      this.transition.age += Math.min(0.1, Math.max(0, dt));
      ...
      camera.quaternion.copy(this.transition.quaternion).slerp(rotation, t);
      camera.fov = THREE.MathUtils.lerp(this.transition.fov, pose.fov, t);
    ...
    this.settled =
      !this.transition &&
      camera.position.distanceTo(pose.position) < 0.025 &&
      camera.quaternion.angleTo(rotation) < 0.01 &&
      Math.abs(camera.fov - pose.fov) < 0.05;
