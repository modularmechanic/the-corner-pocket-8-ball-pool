...
export const PUB_GALLERY_PROPS={
  frame:'models/pub/pub-photo-frame.glb',teamAtlas:'textures/pub/gallery-atlas.webp',eventsAtlas:'textures/pub/pool-events-atlas.webp',
...
function addPoolClubGallery(walls:{front:THREE.Group;right:THREE.Group;back:THREE.Group},installer:PropInstaller,source:THREE.Object3D) {
  ...
  for(const [eraIndex,era] of POOL_DECADE_GALLERIES.entries()){
    ...
    texture.flipY=false;texture.anisotropy=PROP_ANISOTROPY;
    ...
    installer.texture(PUB_GALLERY_PROPS.decadeAtlases[eraIndex],{use:loaded=>{
  ...
  const albumPrint=new THREE.MeshStandardMaterial({name:'Pool champions club album',roughness:.76,envMapIntensity:.12});
  installer.texture(PUB_GALLERY_PROPS.eventsAtlas,{
    prepare:atlas=>{atlas.colorSpace=THREE.SRGBColorSpace;atlas.flipY=false;},
    use:atlas=>{albumPrint.map=atlas;albumPrint.needsUpdate=true;},
  });
...
export function buildPubGallery(room:THREE.Group,installer:PropInstaller,walls:{front:THREE.Group;right:THREE.Group;back:THREE.Group}) {
  ...
  installer.model(PUB_GALLERY_PROPS.frame,{use:frame=>{
    addPoolClubGallery(walls,installer,frame);
    const unused=new Set<THREE.Material>(),prints:THREE.MeshStandardMaterial[]=[];
    for(const entry of entries){
      const variant=frame.clone(true);
      const print=new THREE.MeshStandardMaterial({name:entry.name,color:'#ffffff',roughness:.9,metalness:0,envMapIntensity:.1});prints.push(print);
    ...
    installer.texture(PUB_GALLERY_PROPS.teamAtlas,{
      prepare:atlas=>{atlas.colorSpace=THREE.SRGBColorSpace;atlas.flipY=false;},
      use:atlas=>entries.forEach((entry,index)=>{
        const texture=atlas.clone();texture.repeat.set(.48,.48);texture.offset.set(entry.tile[0]*.5+.01,entry.tile[1]*.5+.01);
        prints[index].map=texture;prints[index].needsUpdate=true;
      }),
    });
  }});
}
