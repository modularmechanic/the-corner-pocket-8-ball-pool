...
export function prewarmPrograms(
  ...
  const previous = renderer.getRenderTarget(),
    target = composer ? new THREE.WebGLRenderTarget(1, 1) : null;
  try {
    if (target) {
...
export class PoolPostprocessing {
  ...
  prewarm(ceiling: RenderBudget) {
    ...
    try {
      ...
      for (const material of [
        ...
        renderer.compile(quad, this.camera);
      }
      ...
      const compileOnly = {
        ...
        setRenderTarget() {},
        render: (object: THREE.Object3D, camera: THREE.Camera) => renderer.compile(object, camera),
      };
      this.output!.render(
        compileOnly as unknown as THREE.WebGLRenderer,
  ...
  render(scene: THREE.Scene, camera: THREE.Camera, dt: number) {
    if (!this.enabled || !this.composer || !this.scenePass) {
