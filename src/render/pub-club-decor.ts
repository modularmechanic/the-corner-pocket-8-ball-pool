...
function clubPrints() {
  ...
  return canvasTexture(1024,1024,ctx=>{
    for(let poster=0;poster<2;poster++){
      const x=poster*512;ctx.fillStyle=poster?'#233c38':'#e1cfa8';ctx.fillRect(x,0,512,768);
      for(let i=0;i<1700;i++){ctx.fillStyle=random()>.5?'#6a462416':'#fff1c214';ctx.fillRect(x+random()*512,random()*768,1+random()*5,1+random()*3);}
      ctx.strokeStyle=poster?'#b8a56a':'#4c5140';ctx.lineWidth=3;ctx.strokeRect(x+23,23,466,720);
      ctx.textAlign='center';ctx.fillStyle=poster?'#e7d7ac':'#32453d';ctx.font='22px Georgia';ctx.fillText('THE CORNER POCKET',x+256,76);
      ctx.font='italic 36px Georgia';ctx.fillText(poster?'The best seat':'Make a night of it',x+256,153);
      ctx.font='bold 67px Georgia';ctx.fillText(poster?'LIVE':'TUESDAY',x+256,250);ctx.fillText(poster?'SPORT':'EIGHTS',x+256,326);
      ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(x+82,363);ctx.lineTo(x+430,363);ctx.stroke();
      ctx.beginPath();ctx.arc(x+256,458,61,0,Math.PI*2);ctx.fill();ctx.fillStyle=poster?'#233c38':'#e1cfa8';ctx.beginPath();ctx.arc(x+256,458,31,0,Math.PI*2);ctx.fill();
      ctx.fillStyle=poster?'#e7d7ac':'#32453d';ctx.font='bold 35px Georgia';ctx.fillText('8',x+256,471);
      ctx.font='27px Georgia';ctx.fillText(poster?'FOOTBALL  ·  RUGBY':'DOUBLES & GOOD COMPANY',x+256,574);
      ctx.font='19px Georgia';ctx.fillText(poster?'Cold drinks. Close games.':'Every Tuesday · From seven',x+256,625);
      ctx.font='italic 18px Georgia';ctx.fillText('Your local, since 1928',x+256,695);
      // A few folded edges and rubbed ink make a printed poster, not a UI card.
      ctx.strokeStyle=poster?'#fff2ca12':'#44352216';ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(x+12,194);ctx.lineTo(x+503,202);ctx.moveTo(x+334,7);ctx.lineTo(x+325,757);ctx.stroke();
    }
    const labels=[['CLUB CHAMPIONS','1998'],['THE CAPTAIN’S CUP','DOUBLES LEAGUE'],['CHARITY OPEN','WINNERS']];
    for(let i=0;i<labels.length;i++){
      const x=i*340;ctx.fillStyle='#b59150';ctx.fillRect(x,808,340,164);ctx.fillStyle='#30291e';ctx.textAlign='center';
      ctx.font='bold 23px Georgia';ctx.fillText(labels[i][0],x+170,867);ctx.font='18px Georgia';ctx.fillText(labels[i][1],x+170,916);
      ctx.strokeStyle='#604d2c';ctx.lineWidth=2;ctx.strokeRect(x+10,818,320,144);
    }
  });
}
...
function neonAtlas() {
  return canvasTexture(1536,512,ctx=>{
    ctx.clearRect(0,0,1536,512);ctx.textAlign='center';ctx.textBaseline='middle';
    const signs=[{x:0,w:1024,label:'WINNERS CLUB',font:105,color:'#f6b65c'},{x:1024,w:512,label:'POOL',font:125,color:'#76d9d1'}];
    for(const sign of signs){
      ctx.font=`${sign.font}px Georgia`;ctx.lineJoin='round';
      ...
      for(const blur of [42,22,9]){ctx.shadowColor=sign.color;ctx.shadowBlur=blur;ctx.strokeStyle=sign.color;ctx.lineWidth=3;ctx.strokeText(sign.label,sign.x+sign.w/2,256);}
      ctx.shadowBlur=0;ctx.strokeStyle=sign.color;ctx.lineWidth=3.8;ctx.strokeText(sign.label,sign.x+sign.w/2,256);ctx.strokeStyle='#fff5dd';ctx.lineWidth=1.2;ctx.strokeText(sign.label,sign.x+sign.w/2,256);
    }
  });
}
