...
export function woodTexture() {
  ...
  walnutSource = canvasTexture(2048, 512, (ctx) => {
    ctx.fillStyle = '#4c2c1c';
    ctx.fillRect(0, 0, 2048, 512);
    for (let i = 0; i < 1900; i++) {
      const y = random() * 512,
        alpha = 0.04 + random() * 0.15;
      ctx.strokeStyle = random() > 0.5 ? `rgba(221,153,81,${alpha})` : `rgba(9,4,2,${alpha * 1.5})`;
      ctx.lineWidth = random() * 2 + 0.3;
      ctx.beginPath();
      ctx.moveTo(0, y);
      for (let x = 0; x <= 2048; x += 32) ctx.lineTo(x, y + Math.sin(x * 0.004 + y * 0.08) * (2 + random() * 2));
      ctx.stroke();
    }
    const glow = ctx.createLinearGradient(0, 0, 0, 512);
    glow.addColorStop(0, '#ffffff18');
    glow.addColorStop(0.4, '#ffffff00');
    glow.addColorStop(1, '#00000044');
    ctx.fillStyle = glow;
    ctx.fillRect(0, 0, 2048, 512);
    ctx.globalCompositeOperation = 'multiply';
    ctx.fillStyle = '#ab673d';
    ctx.fillRect(0, 0, 2048, 512);
  });
  ...
  return canvasTexture(2048, 2048, (ctx) => {
    ...
    for (let row = -14; row < 35; row++)
      for (let col = -6; col < 12; col++) {
        const x = col * (len + w) + (((row % 5) + 5) % 5) * w,
          y = row * w;
        const tone = 24 + random() * 19;
        ctx.fillStyle = `rgb(${tone * 1.28},${tone},${tone * 0.74})`;
        ctx.fillRect(x + 1, y + 1, len - 2, w - 2);
        ctx.fillStyle = `rgb(${tone * 1.2},${tone * 0.92},${tone * 0.69})`;
        ctx.fillRect(x + len + 1, y + 1, w - 2, len - 2);
        for (let n = 0; n < 16; n++) {
          ctx.strokeStyle = `rgba(${random() > 0.5 ? '157,113,71' : '0,0,0'},.12)`;
          ctx.lineWidth = random() + 0.3;
          const lineY = y + random() * w;
          ctx.beginPath();
          ctx.moveTo(x, lineY);
          ctx.bezierCurveTo(x + 80, lineY - 3, x + 200, lineY + 2, x + len, lineY);
          ctx.stroke();
        }
        ctx.strokeStyle = '#77604520';
        ctx.lineWidth = 1;
        ctx.strokeRect(x + 2, y + 2, len - 4, w - 4);
      }
  });
}
export function ballTexture(id: number) {
  ...
  return canvasTexture(1024, 512, (ctx) => {
    ctx.fillStyle = id > 8 ? '#f4efdf' : BALL_COLORS[id];
    ctx.fillRect(0, 0, 1024, 512);
    if (id > 8) {
      ctx.fillStyle = BALL_COLORS[id];
      ctx.fillRect(0, 136, 1024, 240);
    }
    if (id === 0) return;
    for (const x of [256, 768]) {
      ctx.fillStyle = '#fffbed';
      ctx.beginPath();
      ctx.ellipse(x, 256, 80, 82, 0, 0, Math.PI * 2);
      ctx.fill();
      ctx.fillStyle = '#101510';
      ctx.font = `700 ${id > 9 ? 92 : 110}px Arial`;
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText(String(id), x, 263);
    }
  });
}
...
export function clubLightingTexture() {
  const texture = canvasTexture(2048, 1024, (ctx) => {
    ...
    background.addColorStop(0, '#8c988b');
    background.addColorStop(0.48, '#28332d');
    background.addColorStop(1, '#0d0b08');
    ctx.fillStyle = background;
    ctx.fillRect(0, 0, 2048, 1024);
    // Two broad overhead fixtures produce clean reflections on polished resin.
    for (const x of [420, 1400]) {
      const halo = ctx.createRadialGradient(x, 255, 20, x, 255, 250);
      halo.addColorStop(0, '#fff4dbaa');
      halo.addColorStop(1, '#fff4db00');
      ctx.fillStyle = halo;
      ctx.fillRect(x - 250, 5, 500, 500);
      ctx.fillStyle = '#fff9e9';
      ctx.beginPath();
      ctx.roundRect(x - 155, 180, 310, 78, 38);
      ctx.fill();
    }
  });
