...
export const BAIZE_MAPS = [['color', 'textures/table/baize-color.png'], ['normal', 'textures/table/baize-normal.png'], ['surface', 'textures/table/baize-surface.png']] as const;
export const WOOD_SCAN_MAPS = [['map', 'wood-color.jpg'], ['normalMap', 'wood-normal.jpg'], ['roughnessMap', 'wood-roughness.jpg']] as const;
...
export function requestWoodScan(installer: PropInstaller, bind: (slot: WoodScanSlot, scan: THREE.Texture) => void, placeholders?: Record<WoodScanSlot, THREE.Texture>) {
  for (const [slot, path] of WOOD_SCAN_MAPS) installer.texture(path, {
    placeholder: placeholders?.[slot],
    prepare: scan => { scan.wrapS = scan.wrapT = THREE.RepeatWrapping; if (slot === 'map') scan.colorSpace = THREE.SRGBColorSpace; },
...
const tau = Math.PI * 2;
function maps(width: number, height: number, sample: (x: number, y: number) => SurfaceSample, repeatX: number, repeatY: number): SurfaceMaps {
  ...
  const texture = (data: Uint8Array, color = false, format: THREE.PixelFormat = THREE.RGBAFormat) => {
    ...
    result.generateMipmaps = true; result.anisotropy = PROP_ANISOTROPY; result.needsUpdate = true;
    return result;
  };
...
export function createTableSurfaces(installer: PropInstaller) {
  ...
  for (const [kind, path] of BAIZE_MAPS) installer.texture(path, {
    prepare: texture => {
    ...
    use: texture => {
      for (const material of [cloth, cushion]) {
        if (kind === 'color') { material.map = texture; material.color.set(material === cloth ? '#ffffff' : '#e8eddf'); }
        else if (kind === 'normal') material.normalMap = texture;
        else { material.bumpMap = texture; material.roughnessMap = texture; }
  ...
  requestWoodScan(installer, (slot, scan) => {
    const texture = scan.clone(); texture.repeat.set(2, 1); texture.center.set(.5, .5); texture.rotation = Math.PI / 2;
    for (const material of [walnut, sideWood, darkWood]) { material[slot] = texture; material.needsUpdate = true; }
  }, timber);

  random = seededRandom('corner-pocket-brass-v2');
  const metal = maps(512, 512, (x, y) => {
