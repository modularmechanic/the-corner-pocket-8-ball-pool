...
import { acceptedRoom, type ClientToServerEvents, type CommandResult, type Identity, type RoomReply, type RoomSnapshot, type ServerToClientEvents } from './protocol';
import { SnapshotTimeline } from './timeline';
import type { Match, MatchActor, MatchChange, MatchCommand, MatchUpdate } from './types';

export interface RemoteMatchOptions { identity: Identity; url?: string; now?: () => number }
...
export class RemoteMatch implements Match {
  ...
  constructor(config: RemoteMatchOptions) {
    ...
    this.socket.on('disconnect', () => {
      this.generation++; this.inFlight = false; this.recovering = false; this.timeline.clear();
      this.notify({ type: 'connection' });
    });
    this.socket.on('room:replaced', () => { this.leave(); this.notify({ type: 'replaced' }); });
    this.socket.on('room:state', packet => {
      if (!this.roomInfo || packet.code !== this.roomInfo.code) return;
      this.receive(packet); this.notify({ type: 'state' });
    });
  }
  get state(): GameState { if (!this.current) throw new Error('Join a room before reading its match.'); return this.current; }
  get room() { return this.roomInfo; }
  get seat() { return this.ownSeat; }
  get connected() { return this.socket.connected; }
  get pending() { return this.inFlight || this.recovering; }
  ...
  presentation(now = this.now()) { return this.timeline.sample(now) || this.state; }
  drainEvents(): TableEvent[] { return this.muted ? [] : this.timeline.drainEvents(this.now()); }
  private receive(packet: RoomSnapshot, immediate = false) {
    this.roomInfo = packet; this.current = packet.state;
  ...
  private accept(reply: RoomReply): CommandResult {
    ...
    this.ownSeat = reply.seat; this.recovering = false; this.timeline.clear(); this.receive(reply, true); this.notify({ type: 'state' });
    return { ok: true };
  }
  private async recover(code: string) {
    ...
    try {
      ...
      if (!result.ok) { this.leave(); this.notify({ type: 'error', error: result.error! }); }
    } catch { if (generation === this.generation) { this.leave(); this.notify({ type: 'error', error: 'Could not recover the table.' }); } }
    finally { if (generation === this.generation) { this.recovering = false; this.notify({ type: 'connection' }); } }
  }
  private async connect() {
    if (this.socket.connected) return;
    await new Promise<void>((resolve, reject) => {
      const cleanup = () => { clearTimeout(timeout); this.socket.off('connect', ready); this.socket.off('connect_error', failed); };
      const ready = () => { cleanup(); resolve(); };
      const failed = () => { cleanup(); reject(new Error('The room server is unavailable. Please try again.')); };
      const timeout = setTimeout(failed, 8000);
      this.socket.once('connect', ready); this.socket.once('connect_error', failed); this.socket.connect();
    });
  }
  private async enter(send: () => Promise<RoomReply>): Promise<CommandResult> {
    ...
    } catch (error) { return { ok: false, error: error instanceof Error ? error.message : 'Could not open the table.' }; }
    finally { if (generation === this.generation) { this.inFlight = false; this.notify({ type: 'connection' }); } }
  }
  ...
  async execute(command: MatchCommand): Promise<CommandResult> {
    if (!this.roomInfo || !this.connected || command.type !== 'equip' && !this.ready || this.pending) return { ok: false, error: 'Reconnect all players before playing.' };
    if (command.type === 'reset') command = { type: 'rematch' };
    if (['shoot', 'place', 'chalk'].includes(command.type) && !this.actor.canAct) return { ok: false, error: 'Wait for your turn.' };
    const generation = this.generation; this.inFlight = true; this.notify({ type: 'connection' });
    try {
      const socket = this.socket.timeout(7000);
      const reply = command.type === 'shoot' ? await socket.emitWithAck('game:shot', command.shot)
        : command.type === 'place' ? await socket.emitWithAck('game:place', { x: command.x, z: command.z })
        : command.type === 'equip' ? await socket.emitWithAck('game:equip', { cue: command.cue })
        : command.type === 'chalk' ? await socket.emitWithAck('game:chalk', {})
    ...
    } catch { return { ok: false, error: 'Connection interrupted. Reconnecting…' }; }
    finally { if (generation === this.generation) { this.inFlight = false; this.notify({ type: 'connection' }); } }
  }
  leave() { this.generation++; this.inFlight = false; this.recovering = false; this.socket.emit('room:leave'); this.roomInfo = null; this.timeline.clear(); }
  dispose() { this.leave(); this.socket.removeAllListeners(); this.socket.disconnect(); this.listeners.clear(); this.current = null; }
}
