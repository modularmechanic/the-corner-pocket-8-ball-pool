...
export class LocalMatch implements Match {
  ...
  constructor(config: LocalMatchOptions) {
    ...
    this.nextSeed = config.nextSeed || (() => `${config.seed}-${Date.now().toString(36)}-${(++this.revision).toString(36)}`);
  ...
  snapshot(): GameState { return this.game.snapshot(); }
  get ready() { return this.available; }
  get capabilities() { return matchCapabilities(this.state, this.mode, this.ready); }
  get actor(): MatchActor {
    const state = this.state, seat = activeSeat(state);
    return { seat, team: state.turn, controller: this.mode === 'ai' && seat !== 0 ? 'ai' : 'human',
      canAct: this.available && humanControls(state, this.mode, this.seat) && (state.phase === 'ready' || state.phase === 'ball-in-hand') };
  }
  get thinking() { return this.aiPhase !== ''; }
  get aiPreview() {
    if (!this.plan) return null;
    return { angle: this.plan.angle, power: this.plan.power * Math.min(1,this.aiStrokeElapsed/.55) };
  }
  setReady(ready: boolean) { this.available = ready; if (!ready) this.pauseAI(); }
  setDifficulty(difficulty: Difficulty) { this.difficulty = difficulty; this.pauseAI(); }
  ...
  execute(command: MatchCommand) { return Promise.resolve(this.dispatch(command)); }
  ...
  private apply(command: MatchCommand, seat: number, controller: 'human' | 'ai'): CommandResult {
    ...
    if (command.type === 'reset') {
      ...
      this.reset(command.seed, command.options); return { ok: true };
    }
    if (!Number.isInteger(seat) || seat < 0 || seat >= seatCount(state.format)) return { ok: false, error: 'Wait for all players to be connected.' };
    if (command.type === 'equip') {
      if (this.mode === 'ai' && (seat === 0 ? controller !== 'human' : controller !== 'ai')) return { ok: false, error: 'Choose a cue for your own seat.' };
      if (!canEquipCue(command.cue, state.arcade?.level)) return { ok: false, error: 'That cue is locked at this table level.' };
      const ok = this.game.equipCue(seat, command.cue);
      if (ok) { if (controller === 'human' && seat === activeSeat(state)) this.pauseAI(); this.changed(); }
    ...
    if (command.type === 'rematch' || command.type === 'advance') {
      ...
      if (command.type === 'advance' && !canAdvance(state, this.mode)) return { ok: false, error: 'Win this level to advance, or replay the final level.' };
      this.reset(undefined, { level: normalizeLevel((state.arcade?.level || 1) + (command.type === 'advance' ? 1 : 0)) });
      return { ok: true };
    }
  ...
  arrange(state: GameState) { this.game.arrange(state); this.accumulator = 0; this.events = []; this.pauseAI(); this.changed(); }
  ...
  private updateAI(dt: number,cameraReady:boolean) {
    const state = this.state, key = `${activeSeat(state)}:${state.phase}:${state.shotCount}`;
    if (this.aiPhase !== key) {
      this.pauseAI(); this.aiPhase = key;
      const eligible = availableCues(state.arcade?.level);
      const preferences: CueId[] = this.difficulty === 'casual' ? ['maple-control', 'ash-house']
        : this.difficulty === 'expert' ? ['walnut-master', 'ebony-finesse', 'maple-control', 'ash-house']
        : activeSeat(state) % 2 === 0 ? ['ebony-finesse', 'maple-control', 'ash-house'] : ['rosewood-power', 'maple-control', 'ash-house'];
    ...
    if (state.phase === 'ball-in-hand') {
      if (this.aiElapsed >= .75) { const point = choosePlacement(state); this.apply({ type: 'place', ...point }, activeSeat(state), 'ai'); this.pauseAI(); }
      return;
    }
    if (!this.plan) { if (this.difficulty !== 'casual') this.apply({ type: 'chalk' }, activeSeat(state), 'ai'); this.plan = chooseShot(this.state, this.difficulty, this.random); }
    ...
    if(!cameraReady){this.aiCameraStable=0;this.aiStrokeElapsed=0;return;}
    this.aiCameraStable+=dt;
    if(this.aiCameraStable>=.18&&this.aiElapsed>=.35)this.aiStrokeElapsed+=dt;
    if (this.aiStrokeElapsed >= .55) { this.apply({ type: 'shoot', shot: this.plan }, activeSeat(this.state), 'ai'); this.pauseAI(); }
  }
  dispose() { if (this.disposed) return; this.disposed = true; this.game.dispose(); this.events = []; this.listeners.clear(); }
}
