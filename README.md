...
- The 40 pub GLBs are meshopt-compressed and quantized (13.6 MB to 6.5 MB) by `scripts/meshopt-pub-assets.py` with pinned gltf-transform 4.5.0; the loader decodes them with `MeshoptDecoder`. Quantization moves each mesh's offset and scale onto its node, so `tests/pub-placement.test.ts` compares where every pub and table surface is drawn, and its UV range, against a snapshot of the uncompressed assets. It does not check shading from the quantized normals.
