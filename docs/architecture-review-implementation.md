...
| Asset diet | Stool maps at 1K WebP, brick/stone colour and normal maps at 1K, lossless baize. Unused PNG atlases and non-LOD liquor models moved to `art/`. Meshopt on the GLBs was shipped and reverted: its position quantization moved mesh offsets onto node transforms, and the gallery artwork, built from raw frame geometry, collapsed onto the table. Draw-call and triangle benchmarks cannot catch placement, so geometry re-encodes need a transform check. | 37.6 MB loaded before; about 23 MB after the revert. |
