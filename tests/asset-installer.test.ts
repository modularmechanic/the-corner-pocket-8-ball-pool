...
function fakeLoader() {
  ...
  return {
    ...
    parses,
    flush,
    async arrive(url: string, value: THREE.Object3D | THREE.Texture) {
      requests.get(url)!.resolve(value as never);
      await flush();
    },
    async fail(url: string) {
      requests.get(url)!.reject(new Error(`404 ${url}`));
      await flush();
    },
  };
}
function prop() {
  const scene = new THREE.Group(),
    ...
    material = new THREE.MeshStandardMaterial({ map: new THREE.Texture(), transparent: true });
...
function placeholderIn(parent: THREE.Object3D, material = new THREE.MeshStandardMaterial()) {
  const placeholder = new THREE.Group(),
    geometry = new THREE.BoxGeometry();
  placeholder.add(new THREE.Mesh(geometry, material));
...
test('a path requested four times is parsed and prepared once, then instanced for each request', async () => {
  const fake = fakeLoader(),
    props = createPropInstaller(fake.loader),
    room = new THREE.Group();
  let prepared = 0;
  for (let i = 0; i < 4; i++)
    props.model('models/pub/wall-panel.glb', {
      parent: room,
      placements: [{ x: i, y: 0, z: 0 }],
      prepare: () => prepared++,
    });
  ...
  assert.equal(room.children.length, 4);
  assert.equal(props.revision, 4);
...
test('a swap hides, removes and frees the placeholder but keeps resources the room still draws', async () => {
  ...
  const shared = new THREE.MeshStandardMaterial(),
    bar = new THREE.Mesh(new THREE.BoxGeometry(), shared);
  room.add(bar);
  ...
  assert.equal(placeholder.parent, null);
  assert.equal(disposed.geometry, 1);
  assert.equal(disposed.material, 0, 'the bar still draws the shared material');
});
...
test('a failed prop keeps its placeholder, warns once and is reported when settled', async (t) => {
  ...
  assert.equal(placeholder.parent, room);
  assert.equal(placeholder.visible, true);
  assert.equal(disposed.geometry, 0);
  assert.equal(warn.mock.callCount(), 1);
  assert.match(String(warn.mock.calls[0].arguments[0]), /\/models\/pub\/missing\.glb/);
  assert.deepEqual(await settled, { loaded: [], failed: ['models/pub/missing.glb'] });
  assert.equal(props.revision, 0);
});
...
test('a preparation that rejects the source frees it and keeps the placeholder', async (t) => {
  ...
  props.model('models/pub/sports-tv.glb', {
    parent: room,
    placeholder,
    prepare: () => {
...
test('a prop arriving after dispose is freed and never attached', async () => {
  ...
  props.dispose();
  await fake.arrive('/models/pub/cask-stack.glb', scene);
  await fake.arrive('/wood-color.jpg', texture);
  assert.equal(freed, 2, 'shared late results are freed exactly once');
  assert.equal(bound, 0);
  assert.deepEqual(room.children, [placeholder]);
  assert.equal(props.revision, 0);
});
...
test('a dependent load requested while its parent attaches keeps the pub unsettled', async () => {
  ...
  const atlasMaterial = new THREE.MeshStandardMaterial(),
    placeholderMap = new THREE.Texture();
  let placeholderFreed = 0;
  placeholderMap.addEventListener('dispose', () => placeholderFreed++);
  let settled = false;
  props.model('models/pub/pub-photo-frame.glb', {
    use: (frame) => {
      room.add(frame);
      props.texture('textures/pub/gallery-atlas.webp', {
  ...
  assert.equal(props.revision, 1);
  const atlas = new THREE.Texture();
  await fake.arrive('/textures/pub/gallery-atlas.webp', atlas);
  assert.equal(settled, true);
  assert.equal(props.revision, 2);
  assert.equal(atlasMaterial.map, atlas);
  assert.equal(atlas.anisotropy, 8);
  assert.equal(placeholderFreed, 1);
  assert.deepEqual(await props.settled(), {
    loaded: ['models/pub/pub-photo-frame.glb', 'textures/pub/gallery-atlas.webp'],
    failed: [],
  });
});
...
test('a placeholder source shared by two requests is freed only after the last one settles', async (t) => {
  ...
  let freed = 0;
  geometry.addEventListener('dispose', () => freed++);
  const shelfPlaceholder = instancePubModel(source, [{ x: 0, y: 0, z: 0 }]),
    counterPlaceholder = instancePubModel(source, [{ x: 1, y: 0, z: 0 }]);
  shelf.add(shelfPlaceholder);
  counter.add(counterPlaceholder);
  new THREE.Scene().add(shelf, counter);
  ...
  assert.equal(shelfPlaceholder.parent, null);
  assert.equal(freed, 0, 'the counter placeholder still draws the source');
  ...
  const kept = new THREE.BoxGeometry(),
    keptSource = new THREE.Group(),
    room = new THREE.Group();
  keptSource.add(new THREE.Mesh(kept, new THREE.MeshStandardMaterial()));
  let keptFreed = 0;
  kept.addEventListener('dispose', () => keptFreed++);
  const first = instancePubModel(keptSource, [{ x: 0, y: 0, z: 0 }]),
    second = instancePubModel(keptSource, [{ x: 1, y: 0, z: 0 }]);
  new THREE.Scene().add(room);
  ...
  await fake.arrive('/models/pub/stout.glb', prop().scene);
  await fake.fail('/models/pub/missing-stout.glb');
  assert.equal(keptFreed, 0);
  assert.equal(second.parent, room);
});
...
test('settling releases parsed sources without freeing what is drawn, and a later request parses again', async () => {
  ...
  props.texture('textures/table/baize-color.png', {
    use: (texture) => {
      cloth.map = texture;
    },
  });
  let atlas: THREE.Texture | undefined;
  props.texture('textures/pub/pool-1980s-atlas.webp', {
    use: (texture) => {
      atlas = texture;
      texture.dispose();
    },
  ...
  await props.settled();
  assert.equal(atlas, drawnAtlas);
  assert.deepEqual(freed, [], 'settling frees nothing; the atlas was disposed by its own use');
  assert.equal(cloth.map, color);
  assert.equal(room.children.length, 1);
  ...
  await fake.arrive('/textures/table/baize-color.png', recolor);
  await fake.arrive('/models/pub/booth-bench.glb', prop().scene);
  assert.equal(later.map, recolor);
  assert.equal(recolor.anisotropy, 8);
  assert.equal(cloth.map, color);
  assert.equal(room.children.length, 2);
  assert.equal(props.revision, 5);
...
test('a texture placeholder the scene still draws stays allocated', async () => {
  const fake = fakeLoader(),
    props = createPropInstaller(fake.loader),
    room = new THREE.Group(),
    grain = new THREE.Texture();
  new THREE.Scene().add(room);
  room.add(new THREE.Mesh(new THREE.BoxGeometry(), new THREE.MeshStandardMaterial({ map: grain })));
  const { placeholder } = placeholderIn(room);
  let freed = 0;
  grain.addEventListener('dispose', () => freed++);
