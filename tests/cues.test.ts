...
test('five named woods unlock by level and equipment validates IDs, seats and shot phase', () => {
  assert.equal(CUE_CATALOG.length, 5); assert.equal(new Set(CUE_CATALOG.map(cue => cue.wood)).size, 5);
  assert.equal(getCue('__proto__'), undefined); assert.equal(getCue({ id: DEFAULT_CUE }), undefined);
  ...
  try {
    assert.equal(match.dispatch({ type: 'equip', cue: 'maple-control' }, 2).ok, true, 'reserved seats can equip while waiting in a lobby');
    ...
    match.setReady(true); assert.equal(match.dispatch({ type: 'shoot', shot: { angle: 0, power: .03 } }, 0).ok, true);
    assert.equal(match.capabilities.canEquip, false);
    assert.equal(match.dispatch({ type: 'equip', cue: DEFAULT_CUE }, 2).ok, false);
    const state = match.snapshot(); state.phase = 'ball-in-hand'; match.arrange(state);
...
test('default cue preserves strike speed, power cue hits harder and finesse cue produces stronger draw and curve', () => {
  const speeds = new Map<CueId, number>(), draws = new Map<CueId, number>(), curves = new Map<CueId, number>();
  for (const cue of CUE_CATALOG) {
    const straight = isolated(cue.id), draw = isolated(cue.id, true), curve = isolated(cue.id);
    try {
      straight.shoot({ angle: Math.PI / 6, power: .6 });
      const ball = straight.state.balls[0]; speeds.set(cue.id, Math.hypot(ball.vx, ball.vz));
      assert.ok(Math.abs(Math.atan2(ball.vz, ball.vx) - Math.PI / 6) < 1e-10, 'equipment cannot invent a new initial direction');
      const events: TableEvent[] = []; draw.onEvent = event => events.push(event);
      draw.shoot({ angle: 0, power: .45, tipY: -.8 });
      for (let i = 0; i < 100 && !events.some(event => event.kind === 'ball'); i++) draw.step();
      for (let i = 0; i < 4; i++) draw.step();
      assert.ok(events.some(event => event.kind === 'ball')); draws.set(cue.id, -draw.state.balls[0].vx);
      curve.shoot({ angle: 0, power: .4, tipX: .7, elevation: .75 });
      for (let i = 0; i < 60; i++) curve.step();
      assert.equal(curve.state.balls[0].airborne, false); curves.set(cue.id, curve.state.balls[0].z);
    } finally { straight.dispose(); draw.dispose(); curve.dispose(); }
  }
  assert.ok(Math.abs(speeds.get(DEFAULT_CUE)! - (1.4 + .6 * 15)) < 1e-10);
...
test('cue equipment survives snapshot restore and rematches, while lower-level resets remove locked choices', () => {
  ...
  try {
    ...
    const state = match.snapshot(); state.phase = 'over'; state.winner = 0; match.arrange(state);
    assert.equal(match.dispatch({ type: 'rematch' }).ok, true); assert.equal(match.state.cues[2], 'ebony-finesse');
    const ended = match.snapshot(); ended.phase = 'over'; ended.winner = 0; match.arrange(ended);
    assert.equal(match.dispatch({ type: 'advance' }).ok, true); assert.equal(match.state.arcade!.level, 5); assert.equal(match.state.cues[2], 'ebony-finesse');
    ...
    const invalid = match.snapshot(); invalid.cues[0] = 'walnut-master'; match.arrange(invalid);
    assert.equal(match.state.cues[0], DEFAULT_CUE, 'arrangement cannot sneak locked stats into a lower-level table');
  } finally { match.dispose(); }
  const original = isolated('ebony-finesse'), restored = new PoolGame('other');
  try {
    original.shoot({ angle: 0, power: .4, tipX: .7, elevation: .75 }); for (let i = 0; i < 15; i++) original.step();
...
test('AI uses unlocked seat equipment and adjusts power planning to its cue', () => {
  const match = new LocalMatch({ seed: 'cue-ai', mode: 'ai', difficulty: 'expert', options: { level: 5, format: 'doubles' } });
  try {
    const state = match.snapshot(); state.turn = 0; state.teamOrder = [1, 0]; match.arrange(state);
    assert.equal(activeSeat(match.state), 2);
    assert.equal(match.dispatch({ type: 'equip', cue: 'ebony-finesse' }, 2).ok, false, 'a human cannot control their AI partner’s equipment');
    match.update(.1); assert.equal(match.state.cues[2], 'walnut-master');
    assert.ok(match.aiPreview); assert.equal(match.thinking, true);
    const house = match.snapshot(); house.cues[2] = DEFAULT_CUE;
    const stronger = structuredClone(house); stronger.cues[2] = 'rosewood-power';
    assert.ok(chooseShot(stronger, 'expert', () => .5).power < chooseShot(house, 'expert', () => .5).power, 'AI compensates for the power cue instead of blindly overshooting');
  } finally { match.dispose(); }
});
...
test('every cue keeps full-power boosted tricks finite and within its initial energy reservoir', () => {
  for (const cue of CUE_CATALOG) {
    ...
    try {
      edit(game, state => { state.arcade!.hazards = []; state.arcade!.pickups = []; state.arcade!.obstacles = []; state.arcade!.buffs[0].overdrive = 1; });
      game.equipCue(0, cue.id); game.chalkCue(); game.shoot({ angle: .015, power: 1, tipX: .3, tipY: -.7, elevation: .5 });
      const energy = () => game.state.balls.reduce((sum, ball) => sum + (ball.pocketed ? 0 : ball.vx ** 2 + ball.vz ** 2 + (ball.vy || 0) ** 2 + 19.62 * (ball.elevation || 0)), 0);
      ...
      while (game.state.phase === 'rolling' && steps++ < 4000) {
        ...
        assert.ok(game.state.balls.every(ball => [ball.x, ball.z, ball.vx, ball.vz, ball.elevation || 0, ball.vy || 0].every(Number.isFinite)));
      }
