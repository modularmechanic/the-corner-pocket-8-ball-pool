...
let server: ReturnType<typeof attachRooms>, url = '';
before(async () => {
...
after(() => server.close());
const settled = (match: LocalMatch) => { for (let i = 0; i < 4000 && match.state.phase === 'rolling'; i++) match.update(MATCH_STEP, { aiPaused: true }); assert.notEqual(match.state.phase, 'rolling'); };
const finish = (match: LocalMatch) => { const state = match.snapshot(); state.phase = 'over'; state.winner = 0; match.arrange(state); };
...
function inHand(state: GameState): GameState {
  delete state.simulation; Object.assign(state, { shotCount: 3, groups: ['stripes', 'solids'], turn: 1, phase: 'ball-in-hand', shotsLeft: 2 });
  state.balls[0].pocketed = true; Object.assign(state.arcade!, { pickups: [], hazards: [], obstacles: [] });
  return state;
}
...
test('human placement outside the kitchen is rejected with a clear reason only under Old Rules', () => {
  for (const rules of ['old', 'new'] as RuleSet[]) {
    ...
    try {
      ...
      if (rules === 'new') { assert.equal(outside.ok, true); continue; }
      ...
      assert.equal(match.state.phase, 'ready'); assert.equal(match.state.shotsLeft, 2);
    } finally { match.dispose(); }
  }
});

test('the rule set is locked for the session through resets, rematches and new levels', () => {
  const match = new LocalMatch({ seed: 'rules-lock', mode: 'ai', options: { rules: 'old' } }), fallback = new LocalMatch({ seed: 'rules-default' });
  try {
    assert.equal(match.state.rules, 'old');
    assert.equal(match.dispatch({ type: 'reset', options: { rules: 'new', level: 2 } }).ok, true);
    assert.equal(match.state.rules, 'old'); assert.equal(match.state.arcade!.level, 2);
    for (const type of ['rematch', 'advance'] as const) { finish(match); assert.equal(match.dispatch({ type }).ok, true); assert.equal(match.state.rules, 'old'); }
    assert.equal(fallback.state.rules, 'new', 'an engine without a chosen rule set keeps current behaviour');
  } finally { match.dispose(); fallback.dispose(); }
});

test('after a non-scratch Old Rules foul the AI shoots from where the cue ball stopped', () => {
  const match = new LocalMatch({ seed: 'old-ai-lie', mode: 'ai', difficulty: 'expert', options: { rules: 'old' }, random: () => .5 });
  try {
    assert.equal(match.dispatch({ type: 'shoot', shot: { angle: Math.PI, power: .03 } }).ok, true); settled(match);
    const lie = { ...match.state.balls[0] };
    assert.deepEqual([match.state.turn, match.state.phase, match.state.shotsLeft, lie.pocketed], [1, 'ready', 2, false]);
    for (let i = 0; i < 40 && match.state.phase === 'ready'; i++) match.update(.1);
...
test('the AI places a scratched cue ball in the kitchen and plays both of its Old Rules shots', () => {
  const match = new LocalMatch({ seed: 'old-ai-3', mode: 'ai', difficulty: 'expert', options: { rules: 'old' }, random: () => .5 });
  try {
    match.arrange(inHand(match.snapshot()));
    const shots: [number, number][] = [];
    let shotCount = match.state.shotCount, placed = false;
    for (let i = 0; i < 3000 && match.state.turn === 1; i++) {
      const phase = match.state.phase; match.update(.05);
      if (phase === 'ball-in-hand' && match.state.phase === 'ready') { placed = true; assert.ok(match.state.balls[0].x <= HEAD_STRING_X + 1e-9, 'placement is behind the head string'); }
      if (match.state.shotCount !== shotCount) { shotCount = match.state.shotCount; assert.equal(match.state.foul, false); shots.push([match.state.turn, match.state.shotsLeft]); }
    }
    assert.ok(placed);
    assert.deepEqual(shots, [[1, 1], [0, 0]], 'a first-shot miss keeps the table; the second miss passes it');
  } finally { match.dispose(); }
});
...
async function client() {
  const socket = io(url, { forceNew: true, transports: ['websocket'] }); sockets.push(socket);
...
test('a room uses the host’s rule set for every seat, rematch and out-of-kitchen placement check', async () => {
  const host = await client(), guest = await client(), invalid = await client();
  assert.match((await request(invalid, 'room:create', { name: 'Host', token: randomUUID(), rules: 'house' })).error, /Old Rules or New Rules/);
  const created = await request(host, 'room:create', { name: 'Host', token: randomUUID(), rules: 'old' });
  assert.equal(created.state.rules, 'old');
  const joined = await request(guest, 'room:join', { name: 'Guest', token: randomUUID(), code: created.code, rules: 'new' });
  ...
  assert.deepEqual(await request(guest, 'game:place', { x: 1, z: 0 }), { ok: false, error: 'Place the cue ball behind the head string.' });
