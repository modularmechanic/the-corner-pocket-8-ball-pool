...
const shot = (potted: number[] = []): ShotResult => ({
  firstContact: 1,
  potted,
...
test('seeded layouts vary between racks, reproduce exactly and keep props clear of balls and the break lane', () => {
  for (const layout of ['crossfire', 'fortress', 'gauntlet'] satisfies ArenaLayout[]) {
    ...
    for (let seedNumber = 0; seedNumber < 60; seedNumber++) {
      ...
      const seed = `layout-${seedNumber}`,
        arena = createArcade(layout, seed, level),
        balls = newRack(seed);
      ...
      for (const obstacle of arena.obstacles)
        for (const other of arena.obstacles) {
          if (obstacle.id === other.id) continue;
          assert.ok(
            Math.abs(obstacle.x - other.x) >= (obstacle.width + other.width) / 2 ||
              Math.abs(obstacle.z - other.z) >= (obstacle.depth + other.depth) / 2,
      ...
      for (const circle of circles) {
        ...
        assert.ok(
          Math.abs(circle.x) + circle.radius < TABLE.halfWidth && Math.abs(circle.z) + circle.radius < TABLE.halfDepth,
        );
        const laneX = Math.max(-2.85, Math.min(2.55, circle.x));
        assert.ok(
          Math.hypot(circle.x - laneX, circle.z) > circle.radius + TABLE.radius,
          'the break lane must not collect pickups or trigger hazards',
        );
        for (const ball of balls)
          assert.ok(Math.hypot(circle.x - ball.x, circle.z - ball.z) > circle.radius + TABLE.radius);
        for (const other of circles)
          if (circle !== other)
            assert.ok(
              Math.hypot(circle.x - other.x, circle.z - other.z) > circle.radius + other.radius,
              'surface props must not overlap each other',
            );
      }
      ...
      for (const portal of arena.hazards.filter((h) => h.kind === 'portal')) {
        ...
        assert.equal(target?.kind, 'portal');
        assert.equal(target?.link, portal.id);
      }
    }
  }
});
  ...
  for (const power of ['overdrive', 'frost', 'ward', 'focus', 'portal'] satisfies PowerUp[]) {
    ...
    try {
      edit(game, (state) => {
        ...
        state.arcade!.pickups = [{ id: 0, x: -2.1, z: 0, radius: 0.16, available: true, power, expiresAt: 15 }];
      });
      ...
      assert.equal(reward.power, power);
      assert.ok(reward.power);
      assert.ok(!arcade.pickups.some((p) => p.id === 0 && p.available));
      assert.equal(arcade.scores[0], 25);
      assert.equal(
        Object.values(arcade.activeShot).some(Boolean),
        false,
        'a collected effect cannot change an already rolling shot',
      );
      if (reward.power === 'frost') assert.equal(arcade.buffs[1].frozen, 1);
      else if (reward.power === 'portal')
        assert.equal(arcade.portalTurns, 0, 'the new portal pair waits for the rolling shot to finish');
...
test('an untouched stationary ball cannot collect a pickup during another ball’s shot', () => {
  ...
  try {
    ...
    edit(game, (state) => {
      state.arcade!.pickups = [{ id: 0, x: target.x, z: target.z, radius: 0.16, available: true }];
    });
...
test('visible pickups spawn and expire on a deterministic ready-table clock without moving any balls', () => {
  const first = new PoolGame('timed-pickups', { level: 5 }),
    second = new PoolGame('timed-pickups', { level: 5 });
  try {
    const initialBalls = first.snapshot().balls;
    assert.equal(first.state.arcade!.pickups.length, 2);
    const spawned: { at: number; lifetime: number }[] = [],
      expired: number[] = [];
    first.onEvent = (event) => {
      if (event.kind === 'spawn') {
        const a = first.state.arcade!,
          pickup = a.pickups.find((p) => p.id === event.pickup)!;
        ...
        assert.ok(
          first.state.balls.every(
            (b) => b.pocketed || Math.hypot(b.x - pickup.x, b.z - pickup.z) > TABLE.radius + pickup.radius,
          ),
        );
        assert.ok(
          !(pickup.x > -3.41 && pickup.x < 3.26 && Math.abs(pickup.z) < 0.62),
          'spawns must keep the central break lane clear',
        );
      }
    ...
    assert.deepEqual(
      first.snapshot(),
      second.snapshot(),
      'the same seed and elapsed simulation steps reproduce the pickup timeline',
    );
    ...
    first.shoot({ angle: Math.PI, power: 0.03 });
    first.step();
