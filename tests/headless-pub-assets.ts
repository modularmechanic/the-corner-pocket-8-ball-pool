    ...
    void (async () => {
      ...
      if (url.endsWith('.gltf')) {
        ...
        for (const buffer of json.buffers ?? [])
          if (buffer.uri && !buffer.uri.startsWith('data:'))
            buffer.uri = `data:application/octet-stream;base64,${(await fs.readFile(path.join(path.dirname(file), buffer.uri))).toString('base64')}`;
      ...
      this.register((parser) => ({
        ...
        loadTexture(index: number) {
          const texture = new THREE.Texture();
          texture.image = { width: 1024, height: 1024 };
          texture.name = parser.json.textures[index].name ?? `image-${index}`;
