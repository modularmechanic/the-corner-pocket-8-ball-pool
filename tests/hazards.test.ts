...
const portals = (): Hazard[] => [
  { id: 0, kind: 'portal', x: -2, z: 0, radius: .4, link: 1 },
  { id: 1, kind: 'portal', x: 2, z: 0, radius: .4, link: 0 },
];
...
test('a moving ball enters a portal once, preserves speed and emerges clear of the linked portal', () => {
  ...
  try {
    ...
    for (let step = 0; !game.state.balls[0].teleport && step < 120; step++) {
      priorSpeed = speed(game); game.step();
    }
    const cue = game.state.balls[0];
    assert.equal(cue.teleport, 1);
    assert.ok(cue.x >= 2.68 - 1e-5 && Math.abs(cue.z) < 1e-6);
    assert.ok(speed(game) <= priorSpeed && speed(game) > priorSpeed * .98, 'teleportation preserves momentum apart from normal cloth friction');
...
test('a portal refuses transit when all candidate exits are occupied by other balls', () => {
  ...
  try {
    ...
    [0, Math.PI / 4, -Math.PI / 4, Math.PI / 2, -Math.PI / 2, Math.PI].forEach((angle, index) => {
      positions[index + 1] = { x: 2 + Math.cos(angle) * .68, z: Math.sin(angle) * .68 };
    });
    ...
    for (let id = 1; id <= 6; id++) assert.equal(speed(game, id), 0, 'a blocked teleport must never push exit occupants');
  } finally { game.dispose(); }
});
...
test('stationary balls inside portals or electrical pads never move or teleport on another ball’s shot', () => {
  for (const kind of ['portal', 'electric'] as const) {
    ...
    try {
      const hazards = portals(); hazards[0].kind = kind;
      setup(game, { 0: { x: -3, z: .8 }, 1: { x: -2, z: 0 } }, hazards);
      const events: TableEvent[] = [];
      game.onEvent = event => events.push(event);
      ...
      assert.equal(events.some(event => event.kind === 'hazard' && event.ball === 1), false);
    } finally { game.dispose(); }
  }
});
...
test('water and slime add drag, with slime slowing a ball more than water', () => {
  ...
  for (const kind of [null, 'water', 'slime'] as const) {
    ...
    try {
      setup(game, { 0: { x: -2.5, z: 0 } }, kind ? [{ id: 0, kind, x: -2, z: 0, radius: .8 }] : []);
      const events: TableEvent[] = [];
      game.onEvent = event => events.push(event);
      game.shoot({ angle: 0, power: .12 });
      for (let step = 0; step < 24; step++) game.step();
      speeds.push(speed(game));
      if (kind) assert.equal(events.filter(event => event.hazard === kind).length, 1, 'a continuous puddle crossing emits one entry effect');
    } finally { game.dispose(); }
  }
  assert.ok(speeds[0] > speeds[1] && speeds[1] > speeds[2]);
  assert.ok(speeds[0] - speeds[2] > .3, 'slime must have a meaningful slowing effect');
});
...
test('only entering a declared ramp or electric pad can add speed, with a matching visible event and one score award', () => {
  for (const kind of ['ramp', 'electric', 'water', 'slime', 'smoke'] satisfies HazardKind[]) {
    ...
    try {
      setup(game, { 0: { x: -2.5, z: 0 } }, [{ id: 0, kind, x: -2, z: 0, radius: .6, angle: 0 }]);
      const events: TableEvent[] = [];
      game.onEvent = event => events.push(event);
      game.shoot({ angle: 0, power: .12 });
      ...
      if (kind === 'ramp') assert.ok(game.state.balls[0].airborne && game.state.balls[0].vy! > 0, 'a ramp gives the ball a real upward launch');
      for (let step = 0; step < 10; step++) game.step();
      if (kind === 'ramp') assert.ok(game.state.balls[0].elevation! > .1, 'the launched sphere rises above the cloth');
      assert.equal(events.filter(event => event.hazard === kind).length, 1, 'remaining on a pad cannot repeatedly boost or score');
...
test('ramps cannot boost a ball traveling against their direction', () => {
  ...
  try {
    setup(game, { 0: { x: -1.5, z: 0 } }, [{ id: 0, kind: 'ramp', x: -2, z: 0, radius: .6, angle: 0 }]);
    game.shoot({ angle: Math.PI, power: .12 });
    const initial = speed(game); game.step();
...
test('smoke jams the active player only when the moving cue enters the cloud', () => {
  for (const cueInSmoke of [false, true]) {
    ...
    try {
      setup(game, cueInSmoke ? { 0: { x: -2.5, z: 0 } } : { 0: { x: -3, z: .8 }, 1: { x: -2.5, z: 0 } }, [{ id: 0, kind: 'smoke', x: -2, z: 0, radius: .6 }]);
      game.shoot({ angle: cueInSmoke ? 0 : Math.PI, power: .12 });
      if (!cueInSmoke) {
...
test('legal pots award points and a two-ball combo bonus; fouled pots award no pot points', () => {
  for (const count of [1, 2]) {
    ...
    try {
      ...
      if (count === 2) { edit(game,state=>{state.balls[2].vz = 3;}); }
      settle(game);
  ...
  try {
    ...
    edit(game,state=>{state.shotCount = 1;});
    game.shoot({ angle: -Math.PI / 2, power: .2 });
    edit(game,state=>{state.balls[1].vz = 3;});
    settle(game);
...
test('a legally pocketed eight awards the rack-winning bonus', () => {
  ...
  try {
    ...
    edit(game,state=>{state.shotCount = 1;}); edit(game,state=>{state.groups = ['solids', 'stripes'];});
    game.shoot({ angle: -Math.PI / 2, power: .2 });
    settle(game);
