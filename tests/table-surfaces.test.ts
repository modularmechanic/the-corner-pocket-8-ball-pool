...
test('Blender baize loads high-resolution maps once and shares them with cushions', async () => {
  ...
  const texture = (url: string) => new Promise<THREE.Texture>(resolve => {
    const asset = pending.get(url) ?? { texture: new THREE.Texture(), parses: 0, arrive: () => {} };
    asset.parses++; asset.arrive = () => resolve(asset.texture); pending.set(url, asset);
  });
  ...
  try {
    ...
    for (const [name, size] of [['color', 2048], ['normal', 1024], ['surface', 1024]] as const) {
      ...
      const asset = pending.get(`/${path}`)!; assert.ok(asset, `loads ${name} bake`); assert.equal(asset.parses, 1); asset.arrive();
    }
    ...
    assert.equal(walnut.map!.source, pending.get('/wood-color.jpg')!.texture.source, 'the rotated walnut shares the scan upload');
    assert.equal(walnut.map!.colorSpace, THREE.SRGBColorSpace); assert.equal(walnut.map!.rotation, Math.PI / 2);
  ...
  } finally { surfaces.dispose(); installer.dispose(); }
});
