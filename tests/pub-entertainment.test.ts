...
test('three cabinets and two large televisions fit the expanded room outside the cue aisle',()=>{
  assert.equal(PUB_ENTERTAINMENT.slots.length,3);assert.equal(PUB_ENTERTAINMENT.televisions.length,2);
  for(const cabinet of PUB_ENTERTAINMENT.slots){
    assert.ok(cabinet.x>PUB_LAYOUT.bounds.left&&cabinet.x<-15);
    assert.equal(cabinet.y,PUB_LAYOUT.floor);assert.ok(cabinet.z>1&&cabinet.z<11);
    assert.equal(cabinet.rotation,Math.PI/2);assert.ok(cabinet.height>4);
  }
...
test('fictional soccer and rugby broadcasts keep all action in the field across long sessions',()=>{
  for(const sport of ['soccer','rugby']as const){
    ...
    assert.deepEqual(pubBroadcastFrame(sport,3),pubBroadcastFrame(sport,3),'every television runs deterministic channel animation');
    for(let time=0;time<86400;time+=97){
      const frame=pubBroadcastFrame(sport,time);assert.match(frame.clock,/^\d{2}:\d{2}$/);
      for(const point of [...frame.players,frame.ball])assert.ok(Number.isFinite(point.x)&&Number.isFinite(point.y)&&point.x>=0&&point.x<=1&&point.y>=0&&point.y<=1);
    }
  }
});
