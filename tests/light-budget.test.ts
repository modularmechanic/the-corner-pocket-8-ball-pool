...
function lit(scene: THREE.Scene) {
  const result = { point: 0, area: 0 };
  scene.getObjectByName('adaptive-practical-lights')!.traverseVisible((object) => {
...
test('adaptive tiers fade pooled practicals without changing the light counts that key shader programs', () => {
  ...
  for (const [tier, point, area] of [
    ...
    assert.ok(points.every((light) => !light.visible));
    assert.deepEqual(budget.getCounts(), { pointLights: 6, areaLights: 2 });
  }
  budget.configure('auto', 'minimum', 'fast');
  assert.deepEqual(count(scene), { point: 4, area: 2, spot: 3 }, 'a phone ceiling keeps a smaller fixed pool');
...
test('orbiting and source replacement preserve the fixed shader count and finite proxy transforms', () => {
  const { scene, budget, camera } = fixture();
  ...
  for (let frame = 0; frame < 360; frame++) {
    ...
    for (const light of group.children)
      if (light instanceof THREE.PointLight || light instanceof THREE.RectAreaLight) {
...
test('reflection capture restores every original practical and never leaves duplicates enabled after capture', () => {
  ...
  assert.throws(() =>
    budget.withFullLighting(() => {
      assert.deepEqual(count(scene), { point: 14, area: 4, spot: 3 });
...
test('high and ultra pool generously, and disposal restores original source visibility without touching table lamps', () => {
  ...
  budget.configure('high', 'high');
  for (let i = 0; i < 60; i++) budget.update(camera, 1 / 120);
  assert.deepEqual(count(scene), { point: 8, area: 3, spot: 3 });
  assert.deepEqual(lit(scene), { point: 8, area: 3 });
  budget.configure('ultra', 'ultra');
  for (let i = 0; i < 60; i++) budget.update(camera, 1 / 120);
  assert.deepEqual(count(scene), { point: 12, area: 4, spot: 3 });
  assert.deepEqual(lit(scene), { point: 12, area: 4 });
  const proxies = scene
    .getObjectByName('adaptive-practical-lights')!
    .children.filter((child) => child instanceof THREE.PointLight) as THREE.PointLight[];
  budget.configure('auto', 'minimum', 'fast');
  budget.configure('ultra', 'ultra');
  budget.update(camera, 1 / 120);
  assert.ok(
    proxies.slice(4).every((light) => light.intensity < 2),
    'slots re-shown after a lower ceiling fade in rather than pop a stale source',
  );
  budget.configure('auto', 'fast');
  assert.ok(points.every((light) => !light.visible));
  budget.dispose();
...
test('short effect flashes reuse a practical slot without introducing another shader light', () => {
  const { scene, budget, camera } = fixture();
  const flash = new THREE.PointLight('#00ffff', 0, 5, 2);
  flash.position.set(0, 0.5, 0);
  flash.userData.performanceFlash = true;
  scene.add(flash);
  budget.configure('auto', 'minimum');
  budget.update(camera, 2.1);
  flash.intensity = 40;
  budget.update(camera, 1 / 120);
  assert.deepEqual(count(scene), { point: 3, area: 1, spot: 3 });
  const group = scene.getObjectByName('adaptive-practical-lights')!;
  assert.ok(
    group.children.some(
      (light) => light instanceof THREE.PointLight && light.intensity > 30 && light.color.equals(flash.color),
    ),
    'the flash is visible immediately, within the existing slot count',
  );
  flash.intensity = 0;
  for (let i = 0; i < 30; i++) budget.update(camera, 1 / 120);
  assert.deepEqual(count(scene), { point: 3, area: 1, spot: 3 });
  budget.dispose();
});
