...
for (const layout of layouts) {
  test(`${layout}: the opening shot reaches the rack before any cushion or obstacle`, () => {
    ...
    try {
      const cue = game.state.balls[0], apex = game.state.balls[1];
      for (const hazard of game.state.arcade!.hazards) {
        if (hazard.x >= cue.x && hazard.x <= apex.x) {
          assert.ok(Math.abs(hazard.z - cue.z) > hazard.radius + TABLE.radius, 'hazards must not interfere with the opening lane');
        }
      }
      assert.ok(obstacleRay(game.state.arcade, cue.x, cue.z, 1, 0, TABLE.radius) > apex.x - cue.x - TABLE.radius * 2,
        'the initial cue-to-rack lane must be clear');
      for (const ball of game.state.balls) assert.ok(!obstructionAt(game.state.arcade, ball.x, ball.z, TABLE.radius), `ball ${ball.id} must not spawn in an obstacle`);
      ...
      for (let step = 0; step < 120; step++) {
        const beforeX = cue.x;
        game.step();
        if (events.some(event => event.kind === 'ball')) { reachedRack = true; break; }
        ...
        assert.equal(events.some(event => event.kind === 'obstacle' || event.kind === 'cushion'), false);
      }
...
test('stationary balls stay still until physical contact, including at maximum boosted speed', () => {
  ...
  try {
    ...
    const events: TableEvent[] = [];
    game.onEvent = event => events.push(event);
    edit(game,state=>{state.arcade!.buffs[0].overdrive = 1;});
    assert.equal(game.shoot({ angle: 0, power: 1 }), true);
    let contact = false;
    ...
    assert.ok(contact);
    assert.ok(game.state.balls[1].x > .7, 'the struck ball receives the shot momentum');
  } finally { game.dispose(); }
});
...
test('hard breaks across every layout remain finite, dissipate energy, settle and never drift afterward', () => {
  for (const layout of layouts) for (const boosted of [false, true]) for (const angle of [0, .035, -.035]) {
    const game = new PoolGame(`stress-${layout}-${boosted}-${angle}`, { layout, level: 5 });
    try {
      edit(game,state=>{state.arcade!.hazards = [];}); edit(game,state=>{state.arcade!.pickups = [];});
      if (boosted) edit(game,state=>{state.arcade!.buffs[0].overdrive = 1;});
      ...
      while (game.state.phase === 'rolling' && steps++ < 4000) {
        ...
        assert.ok(energy(game) <= previousEnergy * 1.00001 + 1e-7, `${layout} must dissipate energy at every collision step`);
        ...
        for (const ball of game.state.balls) {
          ...
          if (!ball.pocketed) {
            assert.ok(Math.abs(ball.x) <= 5.96 && Math.abs(ball.z) <= 3.15, `${layout}: ball ${ball.id} escaped the live playfield`);
          }
        }
      }
...
test('breakable obstacles lose HP and their destroyed collider no longer blocks the lane',()=>{
  ...
  try {
    const obstacle=game.snapshot().arcade!.obstacles[0],start={x:obstacle.x-obstacle.width/2-1,z:obstacle.z};
    assert.equal(obstacle.maxHp,3);
    const events:TableEvent[]=[];game.onEvent=e=>events.push(e);
    for(const remainingHp of [2,1,0]) {
      fixture(game,{0:start},obstacle.id);
      edit(game,s=>{s.phase='ready';});
      assert.equal(game.shoot({angle:0,power:.2}),true);
    ...
    assert.equal(game.state.arcade!.scores[game.state.turn],obstacle.maxHp*50+events.filter(e=>e.kind==='pickup').length*25);
    assert.ok(events.some(e=>e.kind==='spawn'&&e.x===obstacle.x&&e.z===obstacle.z&&e.power));
    fixture(game,{0:start},obstacle.id);edit(game,s=>{s.phase='ready';});
    const priorHits=events.filter(e=>e.kind==='obstacle').length;
    game.shoot({angle:0,power:.2});for(let i=0;i<45;i++)game.step();
    assert.ok(game.state.balls[0].x>obstacle.x+obstacle.width/2+TABLE.radius,'destroyed blocks must have no invisible collision');
...
test('queued frost weakens exactly the affected player’s next shot', () => {
  ...
  try {
    ...
    edit(game,state=>{state.arcade!.buffs[1].frozen = 1;});
    ...
    game.shoot({ angle: 0, power: 1 });
    assert.ok(Math.abs(speedOfCue() - 16.4 * .65) < 1e-8);
    assert.equal(game.state.arcade!.buffs[1].frozen, 0);
    assert.equal(game.state.arcade!.activeShot.frozen, true);
    function speedOfCue() { return Math.hypot(game.state.balls[0].vx, game.state.balls[0].vz); }
  } finally { game.dispose(); }
});
...
test('ball-in-hand rejects live obstacles and accepts clear felt', () => {
  ...
  try {
    edit(game,state=>{state.phase = 'ball-in-hand';});
...
test('queued Heavy cue adds drag for one shot and Deadeye is consumed into that shot’s preview state', () => {
  ...
  for (const affected of [false, true]) {
    ...
    try {
      ...
      if (affected) { edit(game,state=>{state.arcade!.buffs[0].sticky = 1;}); edit(game,state=>{state.arcade!.buffs[0].focus = 1;}); }
      game.shoot({ angle: 0, power: .2 });
  ...
  assert.ok(speeds[1] < speeds[0] - .2, 'Heavy cue meaningfully increases cloth drag');
});
...
test('scratch shield restores a pocketed cue, while an unshielded scratch leaves it pocketed', () => {
  for (const shielded of [false, true]) {
    ...
    try {
      ...
      if (shielded) edit(game,state=>{state.arcade!.buffs[0].ward = 1;});
      const events: TableEvent[] = [];
      game.onEvent = event => events.push(event);
      game.shoot({ angle: -Math.PI / 2, power: .2 });
      settle(game);
      assert.ok(events.some(event => event.kind === 'pocket' && event.ball === 0));
      assert.equal(game.state.balls[0].pocketed, !shielded);
      if (shielded) assert.ok(!obstructionAt(game.state.arcade, game.state.balls[0].x, game.state.balls[0].z, TABLE.radius));
...
test('scratch shield never rescues a scratch when the eight is also pocketed', () => {
  ...
  try {
    ...
    edit(game,state=>{state.shotCount = 1;});
    edit(game,state=>{state.groups = ['solids', 'stripes'];});
    edit(game,state=>{state.arcade!.buffs[0].ward = 1;});
    game.shoot({ angle: -Math.PI / 2, power: .2 });
    edit(game,state=>{state.balls[8].vz = 3;});
    settle(game);
...
test('all six pockets capture hard and boosted center shots without bouncing back', () => {
  const pockets = [{x:-5.66,z:-2.82},{x:0,z:-2.95},{x:5.66,z:-2.82},{x:-5.66,z:2.82},{x:0,z:2.95},{x:5.66,z:2.82}];
  for (const pocket of pockets) for (const boosted of [false, true]) {
    const game = new PoolGame(`pocket-${pocket.x}-${pocket.z}-${boosted}`);
    try {
      const length = Math.hypot(pocket.x, pocket.z), dx = pocket.x / length, dz = pocket.z / length;
      ...
      assert.equal(events.filter(e => e.kind === 'cushion').length, 0, 'a shot into the mouth cannot bounce before capture');
...
test('fast shots outside the pocket mouth still rebound from the cushion instead of being swallowed', () => {
  for (const x of [-5.05, -.6, .6, 5.05]) for (const side of [-1, 1]) {
...
test('shot geometry validates finite elevation and an in-bounds tip disc without consuming chalk on rejection', () => {
  ...
  try {
    assert.equal(game.chalkCue(), true); assert.equal(game.chalkCue(), false);
    for (const extra of [{ elevation: NaN }, { elevation: -.01 }, { elevation: Math.PI / 3 + .001 }, { tipX: Infinity }, { tipX: .81 }, { tipX: .6, tipY: .6 }]) {
      assert.equal(game.shoot({ angle: 0, power: .5, ...extra }), false);
      assert.equal(game.state.chalked[0], true);
    }
    const events: TableEvent[] = []; game.onEvent = event => events.push(event);
    assert.equal(game.shoot({ angle: .1, power: .5, elevation: .4, tipX: .3, tipY: -.4 }), true);
    assert.equal(game.state.chalked[0], false); assert.equal(game.chalkCue(), false);
    assert.deepEqual(game.state.lastShot, { angle: .1, power: .5, elevation: .4, tipX: .3, tipY: -.4 });
    const cue = events.find(e => e.kind === 'cue')!;
    assert.equal(cue.chalked, true); assert.equal(cue.elevation, .4); assert.equal(cue.tipX, .3); assert.equal(cue.tipY, -.4);
  } finally { game.dispose(); }
});

test('top spin follows and bottom spin draws only after a real cue-ball collision', () => {
  for (const tipY of [-.7, .7]) {
    const game = new PoolGame(`follow-draw-${tipY}`);
    try {
      fixture(game, { 0: { x: -1, z: 0 }, 1: { x: 0, z: 0 }, 2: { x: -3, z: 1.4 } });
      let contact = false; game.onEvent = event => { if (event.kind === 'ball') contact = true; };
      game.shoot({ angle: 0, power: .4, tipY });
      ...
      assert.ok(game.state.balls[0].vx * Math.sign(tipY) > .25, 'cue motion must match the selected follow/draw spin');
      assert.equal(game.state.balls[2].vx, 0); assert.equal(game.state.balls[2].vz, 0);
...
test('side spin changes the cue’s cushion exit while elevated side strikes produce a small bounded curve', () => {
  for (const tipX of [-.7, .7]) {
    const game = new PoolGame(`side-cushion-${tipX}`);
    try {
      fixture(game, { 0: { x: 0, z: 0 } });
      let cushion = false; game.onEvent = event => { if (event.kind === 'cushion') cushion = true; };
      game.shoot({ angle: 0, power: .7, tipX });
      for (let step = 0; !cushion && step < 240; step++) game.step();
      assert.ok(cushion); assert.ok(game.state.balls[0].vx < 0);
      assert.ok(game.state.balls[0].vz * Math.sign(tipX) > .1);
    } finally { game.dispose(); }
  }
  const speeds: number[] = [], lateral: number[] = [];
  for (const elevation of [0, .5]) {
    const game = new PoolGame(`swerve-${elevation}`);
    try {
      fixture(game, { 0: { x: -3, z: 0 } }); game.shoot({ angle: 0, power: .7, tipX: .7, elevation });
      speeds.push(Math.hypot(game.state.balls[0].vx, game.state.balls[0].vz));
...
test('extreme spin and elevated Overdrive breaks remain bounded and settle without moving untouched balls', () => {
  for (const [tipX, tipY, elevation] of [[0, -.8, 0], [0, .8, .6], [.8, 0, .5], [-.5, -.6, Math.PI / 3]]) {
    const game = new PoolGame(`spin-energy-${tipX}-${tipY}`, { level: 5 });
    try {
      edit(game,state=>{state.arcade!.hazards = [];}); edit(game,state=>{state.arcade!.pickups = [];});
      edit(game,state=>{state.arcade!.buffs[0].overdrive = 1;}); game.chalkCue();
      ...
      assert.ok(steps < 4000); assert.equal(energy(game), 0);
    } finally { game.dispose(); }
  }
});
...
test('only the moving white cue ball can collect visible power-ups', () => {
  ...
  try {
    ...
    edit(game,state=>{state.arcade!.pickups = [{ id: 0, x: -1.7, z: 0, radius: .16, available: true, power: 'ward', expiresAt: 15 }];});
    const events: TableEvent[] = []; game.onEvent = event => events.push(event);
    game.shoot({ angle: Math.PI, power: .03 });
    edit(game,state=>{state.balls[1].vx = 3;});
    for (let step = 0; step < 24; step++) game.step();
