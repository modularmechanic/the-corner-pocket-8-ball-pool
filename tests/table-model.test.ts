...
test('table model sizes cloth, cushions and rail caps from the simulated table', () => {
  ...
  const [cloth] = named(model, 'Cloth bed'), clothBox = new THREE.Box3().setFromObject(cloth);
  near(clothBox.max.x, TABLE.halfWidth + TABLE.radius, 'cloth half width'); near(clothBox.min.x, -TABLE.halfWidth - TABLE.radius, 'cloth half width');
  near(clothBox.max.z, TABLE.halfDepth + TABLE.radius, 'cloth half depth'); near(clothBox.min.z, -TABLE.halfDepth - TABLE.radius, 'cloth half depth');
  const cushions = named(model, 'Cushion');
  assert.equal(cushions.length, TABLE_RAILS.length);
  let cushionBackX = 0, cushionBackZ = 0;
  for (const [index, rail] of TABLE_RAILS.entries()) {
    const box = new THREE.Box3().setFromObject(cushions[index]);
    near(box.max.x - box.min.x, rail.halfWidth * 2, 'cushion width'); near(box.max.z - box.min.z, rail.halfDepth * 2, 'cushion depth');
    near((box.max.x + box.min.x) / 2, rail.x, 'cushion x'); near((box.max.z + box.min.z) / 2, rail.z, 'cushion z');
    if (rail.z === 0) { near(Math.abs(rail.x) - rail.halfWidth, TABLE.halfWidth, 'end cushion face'); cushionBackX = Math.abs(rail.x) + rail.halfWidth; }
    else { near(Math.abs(rail.z) - rail.halfDepth, TABLE.halfDepth, 'side cushion face'); cushionBackZ = Math.abs(rail.z) + rail.halfDepth; }
  }
  const caps = named(model, 'Rail cap');
  assert.equal(caps.length, 4);
  for (const cap of caps) {
    const box = new THREE.Box3().setFromObject(cap), end = box.max.z - box.min.z > box.max.x - box.min.x;
    const inner = end ? Math.min(Math.abs(box.min.x), Math.abs(box.max.x)) : Math.min(Math.abs(box.min.z), Math.abs(box.max.z));
    const outer = end ? Math.max(Math.abs(box.min.x), Math.abs(box.max.x)) : Math.max(Math.abs(box.min.z), Math.abs(box.max.z));
    assert.ok(inner > (end ? TABLE.halfWidth : TABLE.halfDepth) + TABLE.radius, 'caps leave the cloth uncovered');
    assert.ok(inner <= (end ? cushionBackX : cushionBackZ) && outer > (end ? cushionBackX : cushionBackZ), 'caps cover the cushion backs');
  }
});

test('table occluders are exactly the objects the table model added to the scene', () => {
  const scene = new THREE.Scene(), pub = new THREE.Group(); scene.add(pub);
  const model = buildTable(scene);
  const ball = new THREE.Mesh(); scene.add(ball);
  assert.deepEqual(new Set(model.occluders), new Set(scene.children.filter(object => object !== pub && object !== ball)));
  ...
  for (const object of model.occluders) object.traverse(child => {
    if (child instanceof THREE.Mesh) assert.ok(child.layers.isEnabled(TABLE_SHADOW_LAYER), 'every table mesh casts table shadows');
