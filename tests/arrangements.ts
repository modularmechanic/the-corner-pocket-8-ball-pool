...
export function arrangeBalls(
  game: PoolGame,
  positions: Record<number, Pick<Ball, 'x' | 'z'> & Partial<Ball>>,
  options: { hazards?: Hazard[]; keepObstacle?: number; arcade?: boolean } = {},
): void {
  const state = game.snapshot();
  delete state.simulation;
  if (options.arcade === false) delete state.arcade;
  if (state.arcade) {
    state.arcade.hazards = options.hazards ?? [];
    state.arcade.pickups = [];
    state.arcade.obstacles.forEach((o) => {
      if (o.id !== options.keepObstacle) o.hp = 0;
    });
  }
  for (const ball of state.balls) {
    ...
    if (position) {
      ...
      ball.airborne = !!ball.elevation || !!ball.vy;
    }
  }
  game.arrange(state);
}
