...
test('buff trail draws from its second point and keeps the newest points in a fixed buffer', () => {
  const trail = new BuffTrail(), geometry = trail.line.geometry, position = geometry.getAttribute('position');
  ...
  try {
    ...
    for (let i = 1; i <= 30; i++) trail.push(i * .1, .03, 0);
    ...
    assert.ok(Math.abs(position.getX(0) - 1.9) < 1e-6 && Math.abs(position.getX(BUFF_TRAIL_CAPACITY - 1) - 3) < 1e-6, 'oldest points shift out');
    trail.clear();
    assert.equal(trail.line.visible, false); assert.equal(geometry.drawRange.count, 0);
    trail.push(5, .03, 5); trail.push(5.1, .03, 5);
    assert.equal(geometry.drawRange.count, 2); assert.equal(position.getX(0), 5);
