...
function openTarget() {
  ...
  Object.assign(state.balls[0], { x: -3, z: 0 });
  Object.assign(state.balls[1], { x: 1, z: 0 });
  return state;
}

test('AI changes its direct shot when a portal interrupts the cue-to-target route', () => {
  const state = openTarget(),
    original = chooseShot(state, 'expert', () => 0.5);
  const portal = {
    id: 0,
  ...
  state.arcade!.hazards = [portal, { id: 1, kind: 'portal', x: -4, z: 2, radius: 0.07, link: 0 }];
  const adjusted = chooseShot(state, 'expert', () => 0.5);
  assert.ok(Math.abs(adjusted.angle - original.angle) > 0.005);
  const dx = Math.cos(adjusted.angle),
    dz = Math.sin(adjusted.angle);
...
test('AI accounts for queued frost, Overdrive and Heavy cue when choosing shot power', () => {
  const base = openTarget(),
    normal = chooseShot(base, 'expert', () => 0.5);
  const powers: Record<string, number> = {};
  for (const effect of ['frozen', 'overdrive', 'sticky'] as const) {
    const state = structuredClone(base);
    state.arcade!.buffs[0][effect] = 1;
    const shot = chooseShot(state, 'expert', () => 0.5);
  ...
  for (const layout of ['crossfire', 'fortress', 'gauntlet'] satisfies ArenaLayout[])
    for (let seed = 0; seed < 4; seed++) {
      ...
      try {
        ...
        arrangement.shotCount = 1;
        arrangement.phase = 'ball-in-hand';
        arrangement.balls[0].pocketed = true;
        ...
        assert.ok(
          game.state.arcade!.hazards.every((h) => Math.hypot(point.x - h.x, point.z - h.z) > h.radius + TABLE.radius),
        );
