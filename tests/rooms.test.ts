...
async function client() {
  const socket: TestSocket = io(url, { forceNew: true, transports: ['websocket'] });
  sockets.push(socket);
  await new Promise<void>((resolve, reject) => {
...
test('two friends share an authoritative seeded table; other turns, extra players and invalid shots are rejected', async () => {
  ...
  assert.equal(created.format, 'singles');
  assert.equal(created.capacity, 2);
  assert.equal(activeSeat(created.state), 0);
  const solo = await request(host, 'game:shot', { angle: 0, power: 0.8 });
  ...
  assert.deepEqual(created.state.balls, joined.state.balls);
  const full = await request(third, 'room:join', { name: 'Third', token: randomUUID(), code: created.code });
  assert.equal(full.ok, false);
  assert.equal((await request(guest, 'game:shot', { angle: 0, power: 0.9 })).ok, false);
  assert.equal((await request(host, 'game:shot', { angle: 0, power: 1000 })).ok, false);
  let hostState: any, guestState: any;
  host.on('room:state', (data) => (hostState = data.state));
  guest.on('room:state', (data) => (guestState = data.state));
  assert.equal((await request(host, 'game:shot', { angle: 0, power: 0.85 })).ok, true);
  assert.equal((await request(host, 'game:shot', { angle: 0, power: 0.85 })).ok, false);
  await delay(250);
  assert.equal(hostState.phase, 'rolling');
  ...
  // Snapshot only once the server has seen the disconnect: the idle pickup clock is frozen from then on.
  guest.disconnect();
  for (let i = 0; room.match.ready && i < 200; i++) await delay(5);
  assert.equal(room.match.ready, false);
  const snapshot = JSON.parse(JSON.stringify(room.match.state));
  ...
  assert.deepEqual(rejoined.state, { ...snapshot, arcade: { ...snapshot.arcade, clock: rejoined.state.arcade.clock } });
...
test('doubles waits for all four seats, advertises teams, and rejects a fifth player and invalid formats', async () => {
  ...
  try {
    ...
    assert.match(invalid.error, /singles or doubles/);
    const created = await request(clients[0], 'room:create', {
      name: 'Seat 0',
      token: randomUUID(),
      format: 'doubles',
    });
    assert.equal(created.format, 'doubles');
    assert.equal(created.capacity, 4);
    assert.equal(created.state.format, 'doubles');
    const room = server.rooms.get(created.code)!;
    const clock = room.match.state.arcade!.clock;
    for (let seat = 1; seat <= 2; seat++)
      await request(clients[seat], 'room:join', { name: `Seat ${seat}`, token: randomUUID(), code: created.code });
    await delay(90);
    assert.equal(room.match.state.arcade!.clock, clock, 'ready timers cannot start with only three seats');
    assert.equal((await request(clients[0], 'game:shot', { angle: 0, power: 0.3 })).ok, false);
    assert.equal((await request(clients[0], 'game:chalk', {})).ok, false);
    const fourth = await request(clients[3], 'room:join', {
      name: 'Seat 3',
      token: randomUUID(),
      code: created.code,
      format: 'singles',
    });
    assert.equal(fourth.seat, 3);
    assert.equal(fourth.format, 'doubles');
    assert.equal(fourth.capacity, 4);
    assert.deepEqual(
      fourth.players.map((p: TeamPacket['players'][number]) => [p.seat, p.team, p.connected]),
      [
        [0, 0, true],
        [1, 1, true],
        [2, 0, true],
    ...
    assert.match(fifth.error, /four players/);
...
test('only the active doubles teammate can shoot, chalk or place the cue', async () => {
  ...
  try {
    for (const seat of [1, 2, 3]) {
      assert.equal((await request(clients[seat], 'game:shot', { angle: 0, power: 0.03 })).ok, false);
      assert.equal((await request(clients[seat], 'game:chalk', {})).ok, false);
    }
    assert.equal((await request(clients[0], 'game:chalk', {})).ok, true);
    assert.equal((await request(clients[0], 'game:shot', { angle: 0, power: 0.03 })).ok, true);
    settleRoom(room);
    assert.equal(room.match.state.phase, 'ball-in-hand');
    assert.equal(activeSeat(room.match.state), 1);
    assert.deepEqual(room.match.state.teamOrder, [1, 0]);
    for (const seat of [0, 2, 3])
      assert.equal((await request(clients[seat], 'game:place', { x: -2.85, z: 0 })).ok, false);
    ...
    assert.equal((await request(clients[3], 'game:shot', { angle: 0, power: 0.03 })).ok, false);
    assert.equal((await request(clients[1], 'game:shot', { angle: 0, power: 0.03 })).ok, true);
  } finally {
    clients.forEach((socket) => socket.disconnect());
  }
});
...
test('a disconnected doubles seat pauses readiness and rejoins its original team and rotation', async () => {
  ...
  try {
    ...
    await until(() => room.seats[1].socketId === null, 'the disconnected seat is marked offline');
    const saved = structuredClone(room.match.snapshot()),
      clock = room.match.state.arcade!.clock;
    await delay(90);
    assert.equal(room.match.state.arcade!.clock, clock);
    assert.equal(
      (await request(clients[3], 'game:place', { x: -2.85, z: 0 })).ok,
      false,
      'a partner cannot take over a disconnected seat',
    );
    outsider = await client();
    assert.equal(
      (await request(outsider, 'room:join', { name: 'Replacement', token: randomUUID(), code: created.code })).ok,
      false,
      'the fourth seat remains reserved for its token',
    );
    returning = await client();
    const rejoined = await request(returning, 'room:join', {
      name: 'Seat 1 returned',
      token: tokens[1],
      code: created.code,
    });
    assert.equal(rejoined.seat, 1);
    assert.equal(rejoined.players[1].team, 1);
    assert.equal(activeSeat(rejoined.state), 1);
    assert.equal(rejoined.format, 'doubles');
    assert.equal(rejoined.capacity, 4);
    assert.deepEqual(rejoined.state.teamOrder, saved.teamOrder);
    assert.deepEqual(rejoined.state.balls, saved.balls);
    assert.equal(rejoined.state.shotCount, saved.shotCount);
...
test('all four clients receive partner rotation and a rematch preserves doubles while resetting the order', async () => {
  ...
  try {
    for (const shooter of [0, 1]) {
      assert.equal((await request(clients[shooter], 'game:shot', { angle: 0, power: 0.03 })).ok, true);
      settleRoom(room);
    ...
    assert.equal(activeSeat(room.match.state), 2);
    assert.deepEqual(room.match.state.teamOrder, [1, 1]);
    assert.equal(
      (await request(clients[0], 'game:shot', { angle: 0, power: 0.03 })).ok,
      false,
      'the previous team shooter cannot steal the next partner’s shot',
    );
    ...
    await until(
      () => packets.every((list) => list.some((packet) => packet.state.seed !== oldSeed)),
      'the rematch reaches all four seats',
    );
    for (const list of packets) {
      const packet = list.filter((p) => p.state.seed !== oldSeed).at(-1)!;
      assert.equal(packet.code, created.code);
      assert.equal(packet.format, 'doubles');
      assert.equal(packet.capacity, 4);
      assert.equal(packet.state.format, 'doubles');
      assert.deepEqual(packet.state.teamOrder, [0, 0]);
      assert.equal(activeSeat(packet.state), 0);
      assert.equal(packet.state.shotCount, 0);
      assert.equal(packet.state.arcade!.level, 2);
      assert.deepEqual(
        packet.players.map((p) => [p.seat, p.team, p.connected]),
        [
          [0, 0, true],
          [1, 1, true],
          [2, 0, true],
...
test('a missing room returns an actionable error and does not create a table', async () => {
  ...
  assert.match(response.error, /not open/);
  assert.equal(server.rooms.size, before);
  socket.disconnect();
});
...
test('friends share seeded layouts, collect visible powers and receive each effect once', async () => {
  ...
  const room = server.rooms.get(created.code)!;
  const arrangement = room.match.snapshot(),
    pickup = arrangement.arcade!.pickups[0];
  pickup.x = -2.1;
  pickup.z = 0;
  room.match.arrange(arrangement);
  ...
  await until(
    () => guestPackets.some((packet) => packet.events.some((event) => event.kind === 'ball')),
    'the guest receives real collision events from the authoritative table',
  );
  await delay(100);
  for (const packets of [hostPackets, guestPackets]) {
    const events = packets.flatMap((packet) => packet.events);
    assert.equal(
      events.filter((event) => event.kind === 'pickup' && event.pickup === pickup.id).length,
      1,
      'a pickup reward is emitted once',
    );
    assert.equal(
      events.filter((event) => event.kind === 'cue').length,
      1,
      'cue audio and effects must not replay in later snapshots',
    );
  ...
  assert.deepEqual(
    hostPackets.flatMap((packet) => packet.events),
    guestPackets.flatMap((packet) => packet.events),
    'both seats receive the same effects',
  );
  ...
  assert.notDeepEqual(
    room.match.state.arcade!.obstacles.map((o) => [o.x, o.z]),
    oldObstacles.map((o) => [o.x, o.z]),
    'a rematch generates a fresh seeded layout',
  );
...
test('chalk is authoritative, validates the shooting seat, and survives rejected spin input', async () => {
  ...
  assert.equal((await request(host, 'game:shot', { angle: 0, power: 0.8, tipX: 1, tipY: 1, elevation: 0 })).ok, false);
  assert.equal(room.match.state.chalked[0], true);
  assert.equal(
    (await request(host, 'game:shot', { angle: 0, power: 0.8, tipX: 0.2, tipY: 0.3, elevation: 0.2 })).ok,
    true,
  );
  assert.equal(room.match.state.chalked[0], false);
  assert.deepEqual(room.match.state.lastShot, { angle: 0, power: 0.8, tipX: 0.2, tipY: 0.3, elevation: 0.2 });
...
test('both friends receive authoritative jump height, vertical velocity and unmodified shot contact fields', async () => {
  ...
  try {
    ...
    guest.on('room:state', (packet) => guestPackets.push(packet));
    const room = server.rooms.get(created.code)!;
    const shot = { angle: 0.015, power: 1, elevation: 0.15, tipX: 0.1, tipY: -0.75 };
    for (const invalid of [
      null,
      { ...shot, elevation: Math.PI / 3 + 0.01 },
      { ...shot, elevation: null },
      { ...shot, tipY: null },
      { ...shot, tipX: 0.6, tipY: -0.6 },
      { ...shot, power: 1.01 },
    ]) {
      assert.equal(
        (await request(host, 'game:shot', invalid)).ok,
        false,
        'invalid jump geometry must be rejected, not stripped or defaulted',
      );
    ...
    await until(
      () =>
        [hostPackets, guestPackets].every(
          (packets) =>
            packets.some((p) => (p.state.balls[0].elevation || 0) > 0.1 && (p.state.balls[0].vy || 0) > 0) &&
            packets.some((p) => p.state.balls[0].airborne && (p.state.balls[0].vy || 0) < -0.3),
        ),
    ...
    for (const packets of [hostPackets, guestPackets]) {
      ...
      assert.ok(
        flying.every((p) => Number.isFinite(p.state.balls[0].elevation) && Number.isFinite(p.state.balls[0].vy)),
      );
      ...
      assert.equal(cue.elevation, shot.elevation);
      assert.equal(cue.tipX, shot.tipX);
      assert.equal(cue.tipY, shot.tipY);
      assert.ok(
        packets.some((p) => p.events.some((e) => e.kind === 'jump' && e.ball === 0)),
        'the launch effect reaches each seat',
      );
    }
    const guestByClock = new Map(
      guestPackets
        .filter((packet) => packet.state.phase === 'rolling')
        .map((packet) => [packet.state.arcade!.clock, packet]),
    );
    let matched = 0;
    for (const packet of hostPackets) {
      ...
      assert.deepEqual(
        packet.state,
        same.state,
        'both clients must receive the identical 3D state at each server tick',
      );
      assert.deepEqual(packet.events, same.events);
      matched++;
    }
    assert.ok(matched >= 3);
  } finally {
...
test('remote match owns room transport, delayed effects, authenticated commands and reconnect recovery', async () => {
  ...
  try {
    ...
    assert.equal(guest.actor.canAct, false);
    ...
    assert.equal(guest.seat, 1);
    assert.equal(guest.state.phase, 'ball-in-hand');
    ...
    await until(
      () => host.state.phase === 'ready' && guest.state.phase === 'ready',
      'authoritative placement reaches both adapters',
    );
...
test('cue choices are seat-owned, level-validated and synchronized through shots, reconnects and rematches', async () => {
  ...
  try {
    ...
    assert.equal(
      (await request(host, 'game:equip', { cue: 'rosewood-power', seat: 1, power: 999, spin: 999 })).ok,
      true,
    );
    assert.deepEqual(
      room.match.state.cues,
      ['rosewood-power', 'ash-house'],
      'payload seat and stat fields cannot alter another seat or the catalog',
    );
    ...
    await until(
      () => packets.some((packet) => packet.state.cues[1] === 'maple-control'),
      'equipment reaches the other players without a shot',
    );
    assert.deepEqual(room.match.state.cues, ['rosewood-power', 'maple-control']);
    assert.equal((await request(host, 'game:shot', { angle: 0, power: 0.5, powerMultiplier: 999 })).ok, true);
    assert.equal(
      (await request(guest, 'game:equip', { cue: 'ash-house' })).ok,
      false,
      'equipment cannot change during an in-flight shot',
    );
    ...
    assert.ok(
      Math.abs(launch.state.balls[0].vx - (1.4 + 0.5 * 15) * 1.08) < 1e-9,
      'the server derives power solely from its cue catalog',
    );
    settleRoom(room);
    guest.disconnect();
    await until(() => room.seats[1].socketId === null, 'equipment seat disconnects');
    returning = await client();
    const rejoined = await request(returning, 'room:join', { name: 'Cue guest', token, code: created.code });
    assert.equal(rejoined.seat, 1);
    assert.deepEqual(rejoined.state.cues, ['rosewood-power', 'maple-control']);
    finishRack(room.match, 0);
    assert.equal((await request(host, 'game:rematch', {})).ok, true);
    assert.deepEqual(room.match.state.cues, ['rosewood-power', 'maple-control']);
    finishRack(room.match, 0);
    ...
    assert.equal(
      (await request(returning, 'game:equip', { cue: 'ebony-finesse' })).ok,
      true,
      'advancing unlocks the next cue on the authority',
    );
  } finally {
    host.disconnect();
    guest.disconnect();
...
test('the build-time room server setting disables, targets same-origin or names a server', () => {
  for (const setting of [undefined, '', '  ', 'not a url', 'ftp://rooms.example', 'http://rooms.example'])
    assert.equal(resolveRoomServer(setting), null, String(setting));
  ...
  assert.deepEqual(
    resolveRoomServer('https://rooms.example/pool/?x=1'),
    { url: 'https://rooms.example' },
    'a path would be read as a Socket.IO namespace',
  );
  for (const local of ['http://localhost:3000', 'http://127.0.0.1:3000'])
    assert.deepEqual(resolveRoomServer(local), { url: local }, 'plain HTTP is only for local servers');
  assert.deepEqual(allowedOrigins(' https://a.example, ,http://b.example:5173 '), [
    'https://a.example',
    'http://b.example:5173',
  ]);
...
test('room transport answers cross-origin browsers only for configured origins', async () => {
  const handshake = async (address: string, origin: string) =>
    (await fetch(`${address}/socket.io/?EIO=4&transport=polling`, { headers: { Origin: origin } })).headers.get(
      'access-control-allow-origin',
    );
  assert.equal(await handshake(url, 'https://friends.example'), null, 'the default server is same-origin only');
  const http = createServer(),
    rooms = attachRooms(http, ['https://friends.example']);
...
test('WebSocket-only browsers from other origins are refused unless configured', async () => {
  const connects = (address: string, origin: string) =>
