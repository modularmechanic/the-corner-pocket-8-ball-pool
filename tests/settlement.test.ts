...
const settle = (state: GameState, shot: Partial<ShotResult> = {}, shooter: 0 | 1 = 0, pendingPortal = false) =>
  settleShot(state, result(shot), {
    legalBefore: legalTargets(state, shooter).map((b) => b.id),
    shooter,
    pendingPortal,
  });
...
test('settlement is pure and a legal own-group count drives score, combo, streak and doubles retention', () => {
  ...
  assert.equal(outcome.state.arcade!.combo, 2);
...
test('foul table covers contact, rail, scratch, illegal break and off-table outcomes without pot points', () => {
  const cases: Partial<ShotResult>[] = [
    { firstContact: null },
    { firstContact: 9 },
  ...
  for (const shot of cases) {
    ...
    if (shot.offTable) {
      assert.ok(outcome.respots.some((p) => p.id === 2));
      assert.equal(outcome.state.balls[2].pocketed, false);
      assert.deepEqual(outcome.state.lastPotted, [1]);
    }
  }
  const state = initialState('illegal-break');
  assert.equal(settle(state, { breakRails: [1] }).state.foul, true);
  const arcade = arranged();
  arcade.shotCount = 0;
  assert.equal(
    settle(arcade, { firstContact: null, breakRails: [], railAfterContact: false, obstacleContact: true }).state.foul,
    false,
  );
});
...
test('ward rescue resets comeback scratches; eight and off-table fouls bypass ward', () => {
  ...
  assert.equal(saved.outcome.wardRescued, true);
  assert.equal(saved.outcome.scratched, false);
  assert.equal(saved.state.foul, false);
  assert.equal(saved.state.arcade!.scratchStreak[0], 0);
  assert.equal(saved.state.balls[0].pocketed, false);
  assert.ok(saved.respots.some((p) => p.id === 0));
  assert.equal(
    saved.events.some((e) => e.reason === 'scratch-streak'),
    false,
  );
  for (const shot of [{ potted: [0, 8] }, { potted: [0], offTable: [2] }, { offTable: [0] }]) {
    const lost = settle(state, shot);
    assert.equal(lost.outcome.wardRescued, false);
    assert.equal(lost.outcome.scratched, true);
    assert.equal(lost.state.foul, true);
  }
});
...
test('comeback awards require consecutive scratches by the same team and reset independently', () => {
  ...
  const event = comeback.events.find((e) => e.reason === 'scratch-streak')!;
  assert.ok(event);
  assert.ok(['overdrive', 'ward', 'focus'].includes(event.status!));
...
test('five-ball streak penalty triggers only on crossing, and a miss resets eligibility', () => {
  const state = arranged();
  state.arcade!.potStreak[0] = 4;
  const crossing = settle(state, { potted: [1] });
  assert.equal(crossing.events.filter((e) => e.reason === 'pot-streak').length, 1);
  const continuing = settle(crossing.state, { firstContact: 2, potted: [2] });
  assert.equal(continuing.events.filter((e) => e.reason === 'pot-streak').length, 0);
  const missed = settle(continuing.state, { firstContact: 3 });
  assert.equal(missed.state.arcade!.potStreak[0], 0);
  const fresh = arranged();
  fresh.arcade!.potStreak[0] = 0;
...
test('eight outcomes include legal wins, early eight, scratch, and break re-racks and respots', () => {
  for (const [potted, offTable, legal, winner] of [
    ...
    assert.equal(ended.state.arcade!.scores[0], winner === 0 ? 500 : 0);
...
test('one portal reward creates a clear stationary pair for exactly two subsequent settlements', () => {
  const state = arranged('portal-lifetime');
  const positions = state.balls.map((b) => [b.x, b.z]);
  const acquired = settle(state, {}, 0, true);
  assert.deepEqual(
    acquired.state.balls.map((b) => [b.x, b.z]),
    positions,
    'portal creation never displaces a ball',
  );
  assert.equal(acquired.state.arcade!.portalTurns, 2);
  const pair = acquired.state.arcade!.hazards.filter((h) => h.kind === 'portal');
  assert.equal(pair.length, 2);
  assert.equal(pair[0].link, pair[1].id);
  assert.equal(pair[1].link, pair[0].id);
  const first = settle(acquired.state);
  assert.equal(first.state.arcade!.portalTurns, 1);
  const second = settle(first.state);
  assert.equal(second.state.arcade!.portalTurns, 0);
  assert.equal(
    second.state.arcade!.hazards.some((h) => h.kind === 'portal'),
    false,
  );
  const renewed = settle(first.state, {}, 0, true);
  assert.equal(renewed.state.arcade!.portalTurns, 2, 'another collected reward renews the pair');
  const ending = settleShot(acquired.state, result({ firstContact: 8, potted: [8] }), {
    legalBefore: [8],
    pendingPortal: true,
  });
  assert.equal(ending.state.arcade!.portalTurns, 0);
  assert.equal(
    ending.state.arcade!.hazards.some((h) => h.kind === 'portal'),
    false,
  );
});
