...
const files = (value: unknown): string[] =>
  typeof value === 'string' ? (/\.\w+$/.test(value) ? [value] : []) : Object.values(value as object).flatMap(files);
...
test('every prop path the render modules request exists under public/', () => {
  ...
  for (const path of paths.filter((path) => path.endsWith('.gltf'))) {
    const gltf = JSON.parse(readFileSync(new URL(`../public/${path}`, import.meta.url), 'utf8')) as {
      buffers?: { uri?: string }[];
      images?: { uri?: string }[];
    };
