...
test('arena visuals add, update and remove obstacles, hazards and pickups by id without leaking', () => {
  ...
  const groups = new Map(
    [...arena.obstacles, ...arena.hazards, ...arena.pickups].map(([id, visual]) => [id, visual.group]),
  );
  arcade.obstacles[0].hp = 0;
  arcade.obstacles[1].hp = 1;
  arcade.pickups[0].available = false;
  update(arcade, 1);
  assert.equal(disposed.size, 0);
  for (const [id, visual] of [...arena.obstacles, ...arena.hazards, ...arena.pickups])
    assert.equal(visual.group, groups.get(id));
  assert.equal(arena.obstacles.get(1)!.group.visible, false);
  assert.equal(arena.pickups.get(20)!.group.visible, false);
  ...
  const removed = [arena.obstacles.get(1)!, arena.hazards.get(15)!, arena.pickups.get(24)!, arena.hazards.get(10)!].map(
...
test('arena visuals rebuild an id whose kind, power or material changes', () => {
  ...
  update(arcade);
  const before = [arena.obstacles.get(1)!, arena.hazards.get(2)!, arena.pickups.get(3)!].map((visual) => ({
    group: visual.group,
    owned: resources(visual.group),
  }));
  arcade.obstacles[0].material = 'steel';
  arcade.hazards[0].kind = 'electric';
  arcade.pickups[0].power = 'ward';
  update(arcade, 1);
