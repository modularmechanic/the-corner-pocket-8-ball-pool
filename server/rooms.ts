...
const reply = (ack: unknown, response: CommandResult) => {
...
export function attachRooms(http: HttpServer, origins = allowedOrigins()) {
  const io = new Server<ClientToServerEvents, ServerToClientEvents, Record<string, never>, SocketData>(http, {
    ...
    allowRequest: (request, callback) =>
      callback(null, originAllowed(request.headers.origin, request.headers.host, origins)),
    ...(origins.length ? { cors: { origin: origins } } : {}),
  });
  const rooms = new Map<string, Room>();
  const syncSeats = (room: Room) =>
    room.match.setReady(
      room.seats.length === seatCount(room.match.state.format) && room.seats.every((seat) => !!seat.socketId),
    );
  const info = (room: Room): RoomSnapshot => ({
    code: room.code,
    format: room.match.state.format,
    capacity: seatCount(room.match.state.format),
    players: room.seats.map((s, seat) => ({ name: s.name, connected: !!s.socketId, seat, team: teamOfSeat(seat) })),
    state: room.match.state,
    events: [],
  });
  const publish = (room: Room) => {
    io.to(room.code).emit('room:state', { ...info(room), events: room.match.drainEvents() });
  };
  ...
  function command(socket: RoomSocket, command: MatchCommand, ack: unknown) {
    ...
    if (!room || seat === undefined || room.seats[seat]?.socketId !== socket.id)
      return reply(ack, { ok: false, error: 'Join a table before playing.' });
  ...
  io.on('connection', (socket) => {
    ...
    const limited = () => {
      const now = Date.now();
      if (now - lastAction < 180) return true;
      lastAction = now;
      return false;
    };
    const sit = (room: Room, seat: number, data: Identity, ack: Ack<RoomReply>) => {
      ...
      if (existing?.socketId && existing.socketId !== socket.id) {
        ...
        if (oldSocket) {
          ...
          delete oldSocket.data.room;
          delete oldSocket.data.seat;
          oldSocket.emit('room:replaced');
        }
      }
      room.seats[seat] = {
        token: data.token,
        name: data.name.trim().slice(0, 22) || (seat ? 'Guest' : 'Host'),
        socketId: socket.id,
      };
      syncSeats(room);
      room.updated = Date.now();
      ...
      ack({ ok: true, seat, ...info(room) });
      publish(room);
    };
    socket.on('room:create', (data, ack) => {
      if (typeof ack !== 'function') return;
      if (limited() || !validIdentity(data)) return ack({ ok: false, error: 'Please wait a moment and try again.' });
      if (data.format !== undefined && data.format !== 'singles' && data.format !== 'doubles')
        return ack({ ok: false, error: 'Choose singles or doubles for this table.' });
      if (data.rules !== undefined && data.rules !== 'old' && data.rules !== 'new')
        return ack({ ok: false, error: 'Choose Old Rules or New Rules for this table.' });
      if (rooms.size >= 120) return ack({ ok: false, error: 'The club is full. Please try again shortly.' });
      leave(socket);
      let code: string;
      do {
        code = randomBytes(5).toString('hex').slice(0, 6).toUpperCase();
      ...
      const room: Room = {
        ...
        match: new LocalMatch({
          seed: code,
          mode: 'online',
          options: data as GameOptions,
          nextSeed: () => code + '-' + randomBytes(3).toString('hex'),
        }),
        seats: [],
        updated: Date.now(),
        broadcastTime: 0,
    ...
    socket.on('room:join', (data, ack) => {
      if (typeof ack !== 'function') return;
      if (limited() || !validIdentity(data) || typeof data.code !== 'string')
        return ack({ ok: false, error: 'Enter a valid six-character room code.' });
      ...
      if (returning === -1 && room.seats.length >= seatCount(room.match.state.format))
        return ack({
          ok: false,
          error: `This table already has ${seatCount(room.match.state.format) === 4 ? 'four' : 'two'} players.`,
        });
      leave(socket);
      sit(room, returning === -1 ? room.seats.length : returning, data, ack);
    });
  ...
  const tick = setInterval(() => {
    ...
    for (const room of rooms.values()) {
      ...
      if (
        (wasRolling && (room.broadcastTime >= 1 / 60 || room.match.state.phase !== 'rolling')) ||
        room.match.hasEvents
      ) {
        publish(room);
        room.broadcastTime %= 1 / 60;
      }
    }
  }, 8);
  const cleanup = setInterval(() => {
    for (const [code, room] of rooms)
      if (room.seats.every((s) => !s.socketId) && Date.now() - room.updated > 10 * 60_000) {
        room.match.dispose();
        rooms.delete(code);
      }
  }, 60_000);
  return {
    ...
    close: async () => {
      ...
      for (const room of rooms.values()) room.match.dispose();
      rooms.clear();
      await io.close();
    },
  };
